...
class RecordFoldsTest {
    ...
    // ── #1108: one mileage basis per job, and per-drop minutes ─────────────

    /**
     * The fielded 2026-09-13 shape (session 499, job …500), in miniature.
     *
     * Two drops from one accept at one store. The platform went straight from drop 1's dropoff
     * confirm to drop 2's, so **drop 2 never stamped a `DELIVERY_ARRIVED`** and has no to-dropoff
     * leg — it folds on the SPAN basis. Both completions are emitted by the same close-out step, in
     * the sweep's order, which puts the LATER-completed drop (D2) first.
     *
     * Before the fix: D2 took the whole 8.8 mi span (which contains D1's 5.0 mi leg) and D1 then
     * claimed that leg again → 13.8 mi for 8.8 mi driven; and D1's minutes were measured from D2's
     * completion — an anchor in D1's own future — giving a NEGATIVE figure.
     *
     * After: the miles are the span, split equally; the TIME partition keeps its meaning (D2 claims
     * the whole gap since DASH_START) and D1, whose completion predates the now-monotonic anchor,
     * folds NULL minutes — the one thing a rate reader can act on honestly.
     */
    @Test
    fun `#1108 - a stacked job whose second drop never arrived shares ONE mileage basis`() {
        val s = "L14"
        val (outcomes, ctx) = foldSession(
            listOf(
                dashStart(s, 1_000_000, odo = 100.0),
                offerAccepted(s, 1_020_000, "h1", eval(net = 12.0, dist = 9.0, opCpm = 0.25)),
                pickupArrived(s, 1_060_000, "J1", "P1", "StoreX", odo = 101.0),   // store leg 1.0
                pickupConfirmed(s, 1_120_000, "J1", "P1", "StoreX", odo = 101.0),
                deliveryArrived(s, 1_700_000, "J1", "D1", odo = 106.0),           // D1 dropoff leg 5.0
                // D2 completed LAST but folds FIRST — the close-out sweep's order.
                delivery(
                    s, 2_200_000, "J1", "D2", totalPay = 6.57, odo = 108.8,
                    phaseStartedAt = 1_760_000,
                ),
                delivery(
                    s, 1_760_000, "J1", "D1", totalPay = 6.58, odo = 108.8,
                    phaseStartedAt = 1_120_000,
                ),
            ),
        )
        val d2 = outcomes[5].delivery!!
        val d1 = outcomes[6].delivery!!

        // ONE basis: both rows are span rows, so neither carries a leg column.
        assertNull("a span-basis row stamps no store leg", d2.milesToStore)
        assertNull(d2.milesToDropoff)
        assertNull("D1's measured leg is swallowed by the span it sits inside", d1.milesToStore)
        assertNull(d1.milesToDropoff)

        // The span (100.0 → 108.8) split equally between the job's two drops.
        assertEquals(4.4, d2.realizedMiles!!, 1e-9)
        assertEquals(4.4, d1.realizedMiles!!, 1e-9)

        val span = ctx!!.lastOdometer!! - ctx.startOdometer!!   // 108.8 − 100.0
        assertEquals(8.8, span, 1e-9)
        assertEquals(
            "Σ per-drop miles is exactly the job's span — no leg is counted twice",
            span, d1.realizedMiles + d2.realizedMiles, 1e-9,
        )

        // Minutes: the partition is unchanged for the row that folds first — the whole gap since
        // DASH_START, exactly as before #1108.
        assertEquals((2_200_000 - 1_000_000) / 60_000.0, d2.realizedMinutes!!, 1e-9)
        // And the sibling, whose completion predates the (now monotonic) anchor, has no attributable
        // partition at all: NULL, never the −7.12 it folded in the field and never a measured 0.
        assertNull(
            "an out-of-order sibling folds null minutes, not a negative and not a fabricated zero",
            d1.realizedMinutes,
        )
        assertNotNull("and the null is not silent — the projector logs this at DEBUG", outcomes[6].note)

        // The anchor never walked backwards: it is the LATEST completion, not the last folded one.
        assertEquals(2_200_000L, ctx.prevDropAt)

        // Net is charged against the apportioned miles, not the doubled ones.
        assertEquals(6.58 - 4.4 * 0.25, d1.netProfit!!, 1e-9)

        // Nothing is left reserved once both drops have folded.
        assertTrue(ctx.legState.spanApportionedMiles.isEmpty())
        assertTrue(ctx.legState.pendingDropoffLegs.isEmpty())
        assertTrue(ctx.legState.dropoffLegJobIds.isEmpty())
    }

    @Test
    fun `#1108 - a reserved share dies with an unassigned sibling (under-attribution, never a double count)`() {
        val s = "L15"
        val (outcomes, ctx) = foldSession(
            listOf(
                dashStart(s, 1_000_000, odo = 100.0),
                offerAccepted(s, 1_020_000, "h1", eval(net = 12.0, dist = 9.0, opCpm = 0.25)),
                pickupArrived(s, 1_060_000, "J1", "P1", "StoreX", odo = 101.0),
                pickupConfirmed(s, 1_120_000, "J1", "P1", "StoreX", odo = 101.0),
                deliveryArrived(s, 1_700_000, "J1", "D1", odo = 106.0),
                delivery(s, 2_200_000, "J1", "D2", totalPay = 6.57, odo = 108.8, phaseStartedAt = 1_760_000),
                // D1 is then abandoned via the help flow — its reservation goes with its leg.
                taskUnassigned(s, 2_300_000, "J1", "D1", TaskPhase.DROPOFF),
            ),
        )
        assertEquals(4.4, outcomes[5].delivery!!.realizedMiles!!, 1e-9)
        assertTrue(
            "an abandoned drop's miles are session-level only (#736) — the reservation is dropped",
            ctx!!.legState.spanApportionedMiles.isEmpty(),
        )
    }

    @Test
    fun `#1108 - a cross-job pending leg is retired by the span but gets no share of this job`() {
        val s = "L16"
        // J2's drop has arrived (its leg closed INSIDE J1's span) when J1's drop completes on the
        // span basis. Fix 1's rule for store legs, applied to dropoff legs: the leg is retired (it is
        // inside a span already counted), but J2's row takes its OWN span delta afterwards — never a
        // share of J1's driving.
        val (outcomes, ctx) = foldSession(
            listOf(
                dashStart(s, 1_000_000, odo = 100.0),
                offerAccepted(s, 1_020_000, "h1", eval(net = 12.0, dist = 9.0, opCpm = 0.25)),
                pickupArrived(s, 1_060_000, "J1", "P1", "StoreOne", odo = 100.4),
                pickupConfirmed(s, 1_120_000, "J1", "P1", "StoreOne", odo = 100.4),
                pickupArrived(s, 1_200_000, "J2", "P2", "StoreTwo", odo = 100.9),
                pickupConfirmed(s, 1_260_000, "J2", "P2", "StoreTwo", odo = 100.9),
                deliveryArrived(s, 1_500_000, "J2", "D2", storeName = "StoreTwo", odo = 102.0),
                // J1's drop: no arrival → span basis, span 100.0 → 103.0.
                delivery(
                    s, 1_800_000, "J1", "D1", storeName = "StoreOne", totalPay = 6.0, odo = 103.0,
                    phaseStartedAt = 1_120_000,
                ),
                // J2's drop, after: its leg was retired, so it takes its own span delta 103.0 → 104.0.
                delivery(
                    s, 1_900_000, "J2", "D2", storeName = "StoreTwo", totalPay = 5.0, odo = 104.0,
                    phaseStartedAt = 1_260_000,
                ),
            ),
        )
        val d1 = outcomes[7].delivery!!
        val d2 = outcomes[8].delivery!!

        assertEquals("J1's span is its own — no sibling of J1 was pending", 3.0, d1.realizedMiles!!, 1e-9)
        assertNull("J2's leg was retired inside J1's span", d2.milesToDropoff)
        assertEquals("so J2's row takes the driving since J1's completion", 1.0, d2.realizedMiles!!, 1e-9)

        val span = ctx!!.lastOdometer!! - ctx.startOdometer!!   // 104.0 − 100.0
        assertTrue(
            "Σ per-drop miles (${d1.realizedMiles + d2.realizedMiles}) must not exceed span ($span)",
            d1.realizedMiles + d2.realizedMiles <= span + 1e-9,
        )
    ...
    @Test
    fun `#1108 - a sequentially-completing stack still partitions its minutes exactly as before`() {
        val s = "L17"
        // The control for the rule above: when the drops DO fold in completion order, nothing about
        // the time partition changes — each drop claims the whole gap since the previous one, which
        // is the quantity `TimeEconomics`/`NetPerHourPair`/`EstimateVsReality` are built on.
        val (outcomes, ctx) = foldSession(
            listOf(
                dashStart(s, 1_000_000, odo = 100.0),
                offerAccepted(s, 1_020_000, "h1", eval(net = 12.0, dist = 9.0, opCpm = 0.25)),
                pickupArrived(s, 1_060_000, "J1", "P1", "StoreX", odo = 101.0),
                pickupConfirmed(s, 1_120_000, "J1", "P1", "StoreX", odo = 101.0),
                deliveryArrived(s, 1_700_000, "J1", "DA", odo = 106.0),
                delivery(s, 1_760_000, "J1", "DA", totalPay = 6.0, odo = 106.5),
                deliveryArrived(s, 2_100_000, "J1", "DB", odo = 108.0),
                delivery(s, 2_200_000, "J1", "DB", totalPay = 6.0, odo = 108.8),
            ),
        )
        val dA = outcomes[5].delivery!!
        val dB = outcomes[7].delivery!!

        assertEquals((1_760_000 - 1_000_000) / 60_000.0, dA.realizedMinutes!!, 1e-9)
        assertEquals((2_200_000 - 1_760_000) / 60_000.0, dB.realizedMinutes!!, 1e-9)
        assertEquals(
            "Σ minutes is still the whole dash up to the last drop — the partition property",
            (2_200_000 - 1_000_000) / 60_000.0,
            dA.realizedMinutes + dB.realizedMinutes,
            1e-9,
        )
        // Both drops measured their legs, so both stay on the leg-sum basis (no apportionment).
        assertEquals(1.0, dA.milesToStore!!, 1e-9)
        assertEquals(5.0, dA.milesToDropoff!!, 1e-9)
        assertEquals(1.5, dB.milesToDropoff!!, 1e-9)
        assertEquals(2_200_000L, ctx!!.prevDropAt)
        assertNull("nothing to note when every row partitions", outcomes[7].note)
    }

    @Test
    fun `#1108 - a completion stamped before the anchor folds null minutes, never negative`() {
        val s = "L18"
        val (outcomes, _) = foldSession(
            listOf(
                dashStart(s, 1_000_000, odo = 100.0),
                // A clock artefact: the completion predates the dash's own start, so there is no
                // partition to measure. Null — a negative reaches a rate and a 0 is a measurement.
                delivery(s, 900_000, "J1", "D1", totalPay = 5.0, odo = 101.0),
            ),
        )
        assertNull(outcomes[1].delivery!!.realizedMinutes)
        assertNotNull(outcomes[1].note)
    }

    @Test
    fun `#1108 - one out-of-order sibling cannot poison the NEXT job's partition`() {
        val s = "L19"
        // The monotonic half, on its own: after the reversed pair the anchor is the LATEST
        // completion, so the next job's drop measures from there. Under the old "take whatever
        // folded last" rule the anchor would have fallen back to 1_760_000 and this row would have
        // claimed 7.3 minutes it did not work.
        val (outcomes, ctx) = foldSession(
            listOf(
                dashStart(s, 1_000_000, odo = 100.0),
                delivery(s, 2_200_000, "J1", "D2", totalPay = 6.0, odo = 108.8),
                delivery(s, 1_760_000, "J1", "D1", totalPay = 6.0, odo = 108.8),
                delivery(s, 2_800_000, "J2", "D3", totalPay = 6.0, odo = 112.0),
            ),
        )
        assertNull("the out-of-order sibling itself", outcomes[2].delivery!!.realizedMinutes)
        assertEquals(
            "the next job measures from the LATEST completion, not the last-folded one",
            (2_800_000 - 2_200_000) / 60_000.0,
            outcomes[3].delivery!!.realizedMinutes!!,
            1e-9,
        )
        assertEquals(2_800_000L, ctx!!.prevDropAt)
    }
}
