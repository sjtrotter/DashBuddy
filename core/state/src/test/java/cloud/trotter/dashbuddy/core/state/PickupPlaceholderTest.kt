...
class PickupPlaceholderTest {
    ...
    // survivor on the region's own pendingOffers, ready for the task edge to mint (was the pre-B3
    // `FlowRegion.pendingOffer` read by the accept edge).
    private fun acceptedOffer(hash: String, stores: List<String>) = PendingOffer(
        offerHash = hash,
    ...
    private fun stepAccept(prev: PlatformRegion, offer: PendingOffer, obs: Observation.FlowObservation): PlatformRegion {
    ...
    fun `single-store offer owns one pickup placeholder carrying the store hint`() {
        val r = stepAccept(region(), acceptedOffer("o1", listOf("H-E-B")), pickupObs(1_000L, "H-E-B"))
    ...
    fun `two-distinct-store offer owns two pickup placeholders`() {
        val r = stepAccept(region(), acceptedOffer("o1", listOf("Sprouts", "CVS")), pickupObs(1_000L, "Sprouts"))
    ...
    fun `same-store two-order offer owns ONE combined pickup but TWO dropoffs (#499 fold)`() {
        val r = stepAccept(region(), acceptedOffer("o1", listOf("H-E-B", "H-E-B")), pickupObs(1_000L, "H-E-B"))
        val job = r.activeJob!!
    ...
    fun `an offer with no parsed orders creates no pickup placeholder (single-delivery fallback)`() {
        val r = stepAccept(region(), acceptedOffer("o1", emptyList()), pickupObs(1_000L, "H-E-B"))
    ...
    fun `add-on at a new store appends a pickup placeholder, a same-store add-on does not`() {
        val r1 = stepAccept(region(), acceptedOffer("o1", listOf("H-E-B")), pickupObs(1_000L, "H-E-B"))
        val r2 = stepAccept(r1, acceptedOffer("o2", listOf("Wendy's")), pickupObs(2_000L, "Wendy's"))
        assertEquals("a new-store add-on brings its own pickup placeholder", setOf("H-E-B", "Wendy's"), pickupHints(r2))

        val r3 = stepAccept(r2, acceptedOffer("o3", listOf("H-E-B")), pickupObs(3_000L, "H-E-B"))
    ...
    fun `FIX3c - a same-store add-on onto a BARE-fallback job makes no duplicate pickup placeholder`() {
        ...
        val r1 = stepAccept(region(), acceptedOffer("o1", emptyList()), pickupObs(1_000L, "H-E-B"))
        ...
        val r2 = stepAccept(r1, acceptedOffer("o2", listOf("H-E-B")), pickupObs(2_000L, "H-E-B"))
        assertEquals(
    ...
    fun `a stacked pickup resolves onto its hint-matching placeholder with the pre-created id`() {
        val r1 = stepAccept(region(), acceptedOffer("o1", listOf("Sprouts", "CVS")), pickupObs(1_000L, "Sprouts"))
    ...
    fun `D3a - a noisy store resolves onto the best-matching slot, not blindly the first open one`() {
        ...
        val r = stepAccept(region(), acceptedOffer("o1", listOf("Sprouts", "CVS")), pickupObs(1_000L, "CVS Pharmacy #123"))
    ...
    fun `a no-match store with 2+ open slots freshly mints (FIX3b - no blind next-open bind)`() {
        ...
        val r = stepAccept(region(), acceptedOffer("o1", listOf("Sprouts", "CVS")), pickupObs(1_000L, "Mystery Mart"))
    ...
    fun `a no-match store with exactly ONE open slot still binds next-open (FIX3b)`() {
        val r = stepAccept(region(), acceptedOffer("o1", listOf("Sprouts")), pickupObs(1_000L, "Mystery Mart"))
        assertEquals("Mystery Mart", r.activeTask?.storeName)
