...
`acceptButton`/`declineButton`/`confirmDeclineButton`/`expandButton` are the four well-known
names the app recognizes today (the SSOT enumeration is the `RuleAction` enum,
`domain/.../action/RuleAction.kt`; `confirmDeclineButton` backs the quick-decline auto-confirm
tap, #577, and is live in `matchers/rules/doordash/offer.json5`):
...
to tap: it emits `AppEffect.PerformRuleAction(action, platform, targetRef, sourceRuleId,
trigger)` from the state machine/EffectMap, and `UiInteractionHandler.performVerifiedClick`
(`:app`) is the only code path that ever taps a third-party app, gated on a granted capability
(content-pinned to the binding's own definition, #422), package scope, and label verification. A
platform supports an action iff its ruleset binds the matching name — adding taps for a new
platform is a rules change, not an app release. See `RuleAction.kt` for the full enumeration +
per-action tap-time verification, and `docs/design/rule-capability-consent.md` for the consent
design (predates `confirmDeclineButton`; check `RuleAction.kt` for the current set).
