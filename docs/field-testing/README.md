...
**Date:** 2026-09-21 → 2026-09-24 (four dashing days) · **Platform(s) tested:** DoorDash (app **8.98.5**
through 09-23 ~15:05, then **8.99.20** — a NEW platform build that rolled out MID-DASH: the last 8.98.5
envelope is `chat_conversation/2026-09-23_15-03-36-147…`, the first 8.99.20 one is
`app_startup/2026-09-23_15-07-05-820…`; DashBuddy's process restarted at 15:06:29 with no log line of any
kind for the 71 s before it — *hypothesis:* the package update restarted our accessibility service —
and recovery replayed 3 observations and resumed `DoorDash:Online` mid-job with no live grace, which is
correct) · **Branch under test:** `master` at `71bacc8c` (build `0.230.0+71bacc8c`, built
2026-09-21T11:53:57Z, read off the logs per PR #1066) — the **first field run of PR #1118** (offer
outcomes on transition evidence). **PR #1125 (#1122/#1123) was NOT on the phone**; the phone was
re-installed on `1b596f74` after this pull (the debug reinstall resets the consent store again — the
dev must re-Allow the automations on first foreground). · **Field conditions:** eleven sessions — 690
(09-21, 3 drops, $76.31), 700 (09-21, 2, $55.34), 707 (09-22, 2, $45.50), 714 (09-22, 1, $14.65), 718
(09-22, a 22-second dash with one decline), 719 (09-23, 4, $82.31), 732 + 736 (09-23 afternoon,
`early_offline`, 0 drops — both REAL dashes whose one accepted order support took away, Bug 3), 739
(09-24, 2, $36.85), 746 (09-24, 1, $18.50), 750 (09-24, 1, $36.89). **67 offer cards: 18 accepts
(DoorDash's own `Offers accepted N/M` counters plus the two support-removed jobs), 48 declines, 1
untouched.** Two pauses (one hand-resumed), no force-stop, no in-app unassign, no return order, no
stacked job. Three process starts (09-21 06:54, 09-23 15:06 mid-dash, 09-25 08:29); every
`PipelineStats` line `restarts=0`, `mappingFailures=0`, `notifListenerDisconnects=0`; **zero ERROR
lines**; no `RecognitionHealth` / `LocaleBoundary` / `TtsHealth` notice. 1 389 capture envelopes (699
on 8.98.5 with 163 UNKNOWN, 405 on 8.99.20 with 100 UNKNOWN — recognition held across the platform
bump); log coverage continuous 09-21 06:54:33 → 09-25 20:49:15 (11 rotations + `app.log`). Pull dir
`~/dashbuddy/logs/2026/09/26`, device purged after verification; five raw-PII envelopes purged from the
pull, pseudonymized copies of the four rule-candidate renders at
`~/dashbuddy/fixtures-pending/2026-09-26/`. Desk report: `~/dashbuddy/logs/2026/09/26/DESK-ANALYSIS.md`.
**Money:** $366.35 reported, $348.50 recorded — ten of eleven sessions reconcile to the cent, session
739 is short exactly $17.85 (Bug 2).

### Bugs

1. **Two new Pledge leaks on DoorDash 8.99.20 — the customer's name on two UNKNOWN surfaces (#1127,
   #1128), plus two known recurrences.** (a) The "Can't hand order to customer" alternate-method page
   now personalises its first step: `com.doordash.driverapp:id/step_title` reads `Contact <First> <I>`
   (2 envelopes, 09-24 16:22:14 / 16:22:31). No rule claims the page; `step_title` is a generic step id
   (its siblings are DoorDash's own step vocabulary) so it is not on `ID_MARKERS`, and `Contact ` is a
   chrome-ambiguous prefix (`Contact support` is a sibling option) so `CustomerTextMarkers` cannot carry
   it. The 8.98.5 render of the same page (`cx_unavailable_steps_view`, 09-23 09:21) said `Contact
   Customer` with no name — **#1127**. (b) The pickup "What do you want to do next?" resolution sheet
   (`Ask the store to place the order` / `Unassign with no pay` / `Order not found in system`) renders
   the fused `For <First> <I>. • <store>` line raw (09-23 15:53:12) — the #809 fused-`For ` class. The
   recognized sibling `pickup_resolution_options` masks the identical line on its own render (`For
   [redacted:de13]`, 09-23 15:07:44), so the gap is the anchor vocabulary — **#1128**. (c) **#1116
   recurred** on 8.99.20 (09-23 15:49:33): the timeline detail sheet's second frame with a street,
   city/ST/ZIP and the quoted note raw; the name line was caught by the `Deliver to ` backstop. (d) The
   **#1122** header-less workflow sheet recurred once on 8.98.5 (09-23 11:30:13) — expected, the fix was
   not on the phone. All five raw files purged; the recognized `dropoff_workflow_sheet` envelope of the
   same job (11:38:20) is fully masked including a hashed bottom-bar name slot. The logs and
   `shareable.log` carry none of the leaked values (grep 0). No raw name in any RECOGNIZED envelope; the
   banking sweep over `captures/` is 0 for all three playbook greps.

2. **#1119 — eight of eighteen real accepts died (44 %), and for the first time one took its money with
   it.** #1118 did its half: the ten accepts it could see are `Accept inferred from the task surface`
   with a job behind each and zero over-inference. The eight it could not see all logged `OFFER_TIMEOUT`
   with a job minted behind them (…-697, -701, -715, -720, -729, -737, -740, -747). Two sub-shapes:
   six are the 09-20 transient-`waiting_for_offer` leave edge (three a plain "Looking for offers"
   frame, three a **HYBRID** frame whose idle header sits over a pickup card that is already painting
   `Delivery for` / `user_name` / `Directions` — the rule still classifies it idle), and **two are
   new: the accept came in the card's last 2–4 s and the `OFFER_EXPIRY` timer (anchored on
   `countdownExpiresAt`) fired ~2 s before the first pickup frame** (…-715 at +46.8 s vs pickup +48.5;
   …-740 at +46.5 vs +49.2) — fail-null by #1104's design, but it is an accept. Six were rescued by
   their receipt (`DROP_SHARE`), one (…-737) was removed by support, and **job …-740 (offer `e6d11317`,
   $17.85) lost its pay**: the drop was completed through the geofence-override / `Mark as delivered`
   path (`dropoff_handoff` → `dropoff_geofence_warning` → `dropoff_help_menu` →
   `dropoff_completed_confirm` → `dash_along_the_way`, 09-24 16:23:33 → 16:23:40) and **DoorDash rendered
   no receipt sheet at all** — 3 of 16 drops this window had no `delivery_summary_*` frame, all three on
   that path (708, 711, 740); 708 and 711 kept their money only because their accept was linked
   (`OFFER_PAY`). With no link there is no fallback: WARN 16:23:50.951 `#691 offer-pay estimate eligible
   but unsplit … ownOfferPay=null` → `payBasis NONE`. *Hypothesis:* DoorDash skips the per-delivery
   receipt on the Mark-as-delivered path (or in along-the-way mode) — unconfirmed. Comment on #1119.
   A `DELIVERY_ADJUSTMENT` for …-740 is the dev's call.

3. **#301 class through a new door — support-chat removal leaves a phantom `PICKUP_CONFIRMED` and a
   15-minute pickup dwell row, twice.** Sessions 732 and 736 each accepted one H-E-B shop order, reached
   the store, went `pickup_issue_menu` → `pickup_resolution_options` → support chat, and support removed
   the order (the dasher never took the in-app `Unassign with no pay` path). Each left seq 3167 / 3178
   `PICKUP_CONFIRMED` (`confirmedAt` = the `dash_along_the_way` frame after a `Missed Delivery` push at
   15:14:20, dwell 16.1 min; and the NEXT offer's presentation instant at 16:03:46 while we still showed
   the dasher at the store, dwell 14.5 min), each committed by a 10 s `GRACE_COMMIT` and emitted at the
   dash end; **zero `TASK_UNASSIGNED`**, no `JOB_ACCEPT_MISMATCH`, offer `d58fc055` left with
   `linkedJobId` NULL, ~18 odometer miles attached to no delivery. *Hypothesis:* no screen on this path
   carries an unassign anchor — the only artifacts are the push and the fresh offer. The 8.99.20 sheet
   where `Unassign with no pay` now lives is unrecognized (#1128). Comment on #301.

4. **#1069 fielded — two DIFFERENT H-E-B offers merged into one row, and the second was never
   spoken.** Offer `8cabdfe5` ($19.50) was HUD-declined at 09-21 14:52:14, its confirm sheet seen at
   14:52:15.9; every frame after it was flow-less (`side_nav_drawer`, `ratings`, UNKNOWN), so it stayed
   pending. A NEW offer `8a540c6e` ($20.50) arrived at 14:52:30.276 and was folded into it as an
   enrich-as-variant, because **all 48 H-E-B offers in the window share `presentationKey`
   `e30e80e8…`** (`sha256(store|orders.size|orderTypes)` is identical for every single-store H-E-B shop
   order). Ledger: `OFFER_RECEIVED 8cabdfe5 $19.50` (seq 3019) → `OFFER_DECLINED 8a540c6e $20.50` (seq
   3020, dt 27.4 s on the FIRST offer's `presentedAt`); session 700 counts 6 offers against DoorDash's
   7; the second got a chat card and a screenshot but **no TTS** (66 `speaking` for 67 `Rule LOG`
   receipts). Comment on #1069.

5. **Three real declines recorded as `OFFER_TIMEOUT` because a flow-less screen held the exit past the
   15 s sheet window** — the dev's "Dash Control" observation, characterized under *Field UX context*
   (two on `on_dash_map` at 16.71 s and 22.46 s; one on `navigation_generic`'s flow-less branch at
   20.75 s). Comment on #1104. Decline recall otherwise 44/48, none misread as an accept.

### Field UX context

- **The dev's observation, characterized.** "When dashing along the way and I decline an offer, it
  sometimes drops me into the Dash Control screen instead of back onto the map; it doesn't record
  accept/decline/timeout until I move back onto the map (Return to dash)." The screen is
  `doordash.screen.on_dash_map` (`dash-lifecycle.json5`, priority 110) — the side-menu home with the
  `This week` earnings pill, `You're dashing now`, a `Return to dash` button and the Home / Schedule /
  Account / Ratings / Earnings / Promos / Preferences / Help list (e.g.
  `on_dash_map/2026-09-23_15-32-49-226…973816`). Its `state` is `modeHint: online` only — **no flow** —
  so `OfferLifecycle.stepOffers` treats it (and the `side_nav_drawer` that follows) as inert, and the
  pending offer resolves on the first flow-bearing frame after Return to dash, which was
  `waiting_for_offer` every time (the `Dispatch` card posts ~30 ms after it). Five occurrences, every
  one a decline made during along-the-way navigation (`dash_along_the_way` → `navigation_generic` was
  the frame before each card): sheet→exit 8.19 s, 11.14 s, 11.40 s (recorded DECLINED) and **16.71 s,
  22.46 s (past `declineSheetWindowMs` = 15 s → `OFFER_TIMEOUT`, HUD "Offer Timed Out!")**. A sixth
  decline went the same way through `navigation_generic`'s flow-less **branch 1** (a frame carrying
  `exit` but no `min`/`mi`, so not branch 0's `flow: idle`): 20.75 s → TIMEOUT, where a full branch-0
  nav frame four minutes earlier resolved a decline in 2.2 s. No `Return to dash` click envelope was
  captured (a Compose control). The same flow-less dwell is what let Bug 4's second offer land on a
  still-pending predecessor. *Possibilities, hypotheses only — the dev has said they are not sure they
  want anything changed:* (a) the window is anchored on the LAST sheet sighting rather than on the
  card's disappearance, so any flow-less dwell after the sheet eats into it; (b) Dash Control / nav
  branch 1 could carry a flow, but that is a state-machine change riding on an offer-outcome fix;
  (c) #1119's evidence-linking design (DECLINED only on direct evidence, NOT_TAKEN at dash end)
  changes this whole path anyway. Why DoorDash lands on Dash Control only sometimes is not visible in
  the data — other along-the-way declines returned to nav normally (09-22 19:52–19:56).
- **The dev's "some declines now produce click envelopes" is not borne out by the pull.** Exactly one
  decline-button click envelope exists (`accessibility.click/initial_decline/2026-09-22_19-51-05-598…`,
  offer 3801fdbc), and it landed **216 ms after DashBuddy's own automated `decline_offer` tap**
  (`Single verified candidate … clicking it` at 19:51:05.366) — *hypothesis:* the event from our own
  `performAction`, not a human tap. Zero confirm-button clicks, zero Accept clicks. Every recorded
  outcome this window came from #1118's transition inference.
- **The confirm-decline capability is still un-granted:** 49 × `Denied confirm_decline — no granted
  capability (fail closed)`, one per sheet sighting. And the reinstall on `1b596f74` reset the store
  again — re-Allow on first foreground.
- **DoorDash 8.99.20 rolled out mid-dash on 09-23** and recognition held (UNKNOWN share 23 % → 25 %).
  The first 8.99.20 `pickup_pre_arrival` after an accept produced one `ParseHealth` WARN ("all 5
  extractable fields unresolved", 09-22 18:00:01 — actually still 8.98.5; a pre-render frame).

### Verification (desk)

- **#1118 / #1104 transition outcomes — CONFIRMED 1/2** on its first run: 10/10 `OFFER_ACCEPTED` are
  task-surface inferences with a job behind each (no over-inference); 44/44 `OFFER_DECLINED` are
  confirm-sheet inferences, all inside the window (median ≈ 5 s, max 11.40 s); the one untouched offer
  (`74695b74`, 09-21 10:03) is a genuine `OFFER_EXPIRY` timeout at +45.1 s. Residuals are Bugs 2, 4, 5.
- **#1114 Compose offer card — CONFIRMED 2/2, RETIRED (moved here).** 67 cards → 67 `Rule LOG
  [OFFER_RECEIVED]` on BOTH 8.98.5 and 8.99.20; 66 `offer_records` (one merged, Bug 4 — a separate
  #830 defect); zero parse nulls on pay / distance / merchant / est. time / score / action / quality;
  two-store offers name both stores (`CVS & McDonald's`); `parseShortfall{offer_popup…}` absent; 30
  offer-shaped UNKNOWN frames are all Accept-less half-renders inside a recognized offer's window; the
  two `side_nav_drawer` frames carrying `Decline` + a route summary are decline-transition overlays of
  already-recognized offers; 54 `NEW_ORDER` lines.
- **#1102 receipt auto-expand (retired 09-21) — re-observed clean:** 14 `Performing expand_earnings`,
  11 `Single verified candidate … clicking it` first try + 1 on a retry; at 09-24 20:26:23.027 a `Could
  not find any live node` was **followed 543 ms later by a successful tap** — the #1102 throttle
  restore working in the field; 0 `Throttled`; one `NONE passed verification` (09-21 15:51:22) where the
  receipt was itemized anyway (`DROP_SHARE`).
- **#1120 / #1033 layer 2 — not exercised, no damage:** 13 `DELIVERY_RECEIPT_REPRICE` events, all
  single-drop single-offer jobs, all `already holds … — no-op`, `receiptRepricedAt` NULL everywhere; no
  stacked job in the window.
- **#1103 — clean negative (3rd):** 0 `receipt running-total read refused`; the three zero-drop sessions
  report NULL; the 3 `SESSION_PAY_SETTLE` fires each inside their own session; back-to-back starts
  707→714 (38 s) and 746→750 (20 s) each carry only their own total.
- **#1108 — clean, not a confirmation:** `Σ realizedMiles ≤ span` on every session (largest gap 2.44 mi,
  session 707), zero negative `realizedMinutes`, no stacked job.
- **Decline target bound (8.97.8+) — bind half holds a second time** (33 × `Single verified candidate
  for decline_offer`, 0 `No 'declineButton' target bound`, no `offer_popup` bindShortfall); confirm half
  still unmeasured (un-granted).
- **#1063 — desk half clean a 3rd time** (67 cards, one presentation each, zero `Replaced by new
  offer`/`Superseded`, `accept_decline_footer_container` in 0 UNKNOWN frames).
- **#1058 — partial:** the alcohol pre-arrival card (`alcohol_dropoff_ic_scan`) claimed as
  `dropoff_pre_arrival` 4× with no raw PII; the workflow sheet recognized + masked once, its header-less
  frame leaked again (#1122, fix not on the phone).
- **#1059 — not exercised** (0 scanner ids tree-wide); adjacent positive: `sensitive.id_verification`
  dropped 39 frames across the two alcohol drops, no scanner frame reached disk; `sensitive.red_card` 0
  on Red-Card shop orders (no over-match).
- **Unexercised:** #1122/#1123 (build predates the fix), #1107 (no `dropoff_step_instructions` folder
  and no `description_text_view` node on either DoorDash build — the wrapper may be gone), #998 (the
  "Can't hand order" page rendered twice with its `Return the order to merchant` step, but both drops
  completed normally), #1121 (no `+$` / `+N stop` text anywhere), #1054 grace-restart (one mid-dash
  restart with no live grace — `Recovery re-armed` correctly absent; summary ends 2.501–2.513 s,
  early-offline ends 10.002–10.004 s) and #1054 pause-relaunch (two `DASH_PAUSED`, one
  `MODE_RESUME_COMMIT` 09-24 16:25:35, no restart mid-pause), #301 deliberate path (Bug 3 is the
  support-chat variant).
- **Censuses.** WARN by tag: `Effects` 83 (49 confirm-decline denials, 29 `GRACE_COMMIT` by design, 1
  `MODE_RESUME_COMMIT`, 4 expand-tap lines above); `Pipeline` 31 (16 `Capture backstop: UNKNOWN screen
  carried customer PII` — `Pickup for ` ×7, `user_name` ×7, `Deliver to ` ×3, `address_line_2` ×1; 13
  `Capture scrubbed` — DasherDirect ×4, Transfer-out ×4, Crimson ×3, `shape:` ×2 — all on the four
  post-dash cash-out visits); `ParseHealth` 9; `Odometer` 5 (4 POOR_ACCURACY 69–419 m, 1
  IMPLAUSIBLE_SPEED 09-22 17:55:50, each recovered); `StateMachine` 1 (Bug 2's #691 line). ERROR 0.
  Sensitive gate drops (DEBUG): dasher_direct 406, catchall 37, id_verification 39, balance 18,
  crimson_balance 10 — all at cash-out moments or the alcohol ID scans. `shareable.log`: 0
  `[scrubbed:…]`. TTS 66 `speaking`, 0 losses. 16 `DELIVERY_COMPLETED` = 16 rows, one per job; no
  doubled drop, no `JOB_ACCEPT_MISMATCH`. parseShortfall vs the benign baseline: `waiting_for_offer`,
  `dash_along_the_way`, `idle_map`, `pickup_shopping` as documented; off-baseline singles
  `pickup_pre_arrival` ×1 (the pre-render frame above) and `bindShortfall{ratings#ratingsHubContent=1}`
  (off-dash 09-22 08:32). UNKNOWN families (263 window frames): 30 offer half-renders (#595), 24
  text-free skeletons (9 of them `drop_off_workflow_host_fragment`), ~23 support-flow **Order details**
  item sheets (`Close sheet` + `Drag handle` + `N items` + `N x` rows, no PII), 16 acceptance-rate /
  offer-history pages (no PII), 9 prism bottom sheets, 7 `This dash so far / Total online time`
  sheets, 7 nav-container frames, 6 `Current dash / Pick up by` task cards (name scrubbed by the
  `user_name` backstop), 6 `Shop and Deliver` headers, 5 `by HH:MM • H-E-B` timeline rows (`Pickup for `
  backstop fired), 3 `Did we resolve your issue?`, 3 `Where did you find it?`, 3 `Can't hand order to
  customer` (Bug 1a), 1 resolution sheet (Bug 1b), 1 #1116, 1 #1122. Click UNKNOWN (188) is
  `Confirm`/`Take photo`/`Scan again`/`Continue`/`This offer`/`End dash`/nav-step taps.

### Open questions

- **The decline window after a flow-less exit** (Bug 5 / Field UX): anchor on card-vanish, give
  `on_dash_map` and nav branch 1 a flow, or leave it to #1119's evidence-linking? The dev has not
  asked for a change; the comment on #1104 lists the options.
- **`DELIVERY_ADJUSTMENT` for job …-740** ($17.85, receipt-less Mark-as-delivered path, Bug 2)?
- **The receipt-less completion path** — does DoorDash really skip the per-delivery receipt on the
  geofence-override / Mark-as-delivered path, or only in along-the-way mode? Three sightings, all
  along-the-way. If confirmed, the offer link is the ONLY pay source on that path (see #1119).
- **Rule candidates from the census** (no PII unless noted): the "Can't hand order to customer" page
  (#1127, PII), the pickup resolution sheet (#1128, PII — and the natural anchor for #301's deliberate
  path), the support-flow Order-details item sheet (23 frames), the acceptance-rate / offer-history
  pages (16), the `This dash so far` progress sheet (7).
- **Is the "Drop off steps" wrapper (#1107) gone on 8.98.5+?** Two pulls without a single render.

### Meta

- DoorDash 8.99.20 arrived mid-dash and the process restarted with it (no log for 71 s, then a clean
  recovery with 3 replayed observations). The corpus has no 8.99.20 fixtures yet; the intake of this
  slice is the next corpus PR.
- Consent reset on every debug reinstall remains a per-dash chore for the dev (4 automations to
  re-Allow); the 49 confirm-decline denials this window are the cost of the confirm half staying
  un-granted.

## 2026-09-20 (desk analysis of the 09-21 pull — the Compose offer card works perfectly and four real accepts still died; a receipt re-priced a drop it had never described; two Pledge leaks)

**Date:** 2026-09-20 · **Platform(s) tested:** DoorDash (app **8.98.5** — a NEW platform build; the
09-16→19 window was 8.97.8, so every reading below is against a tree nothing in the corpus has seen) ·
**Branch under test:** `master` at `aab8d960` (build `0.230.0+aab8d960`, built 2026-09-20T14:02:35Z,
read off the logs per PR #1066) — the **first field run of PR #1115**, the Compose offer-card
recognition fix. **PR #1118 (offer outcomes on transition evidence) was NOT on the phone**, so every
accept and every decline logs `OFFER_TIMEOUT`; that is the known pre-#1118 state, not a new defect. ·
**Field conditions:** one long DoorDash day — session **661** 09:08:52 → 16:40:25 (`summary_screen`,
$215.12 reported, 8 drops, 71.00 mi span) plus `early_offline` **685** and evening session **686**
($31.37, 1 drop); the log window opens on 09-19's sessions 656/660, already covered by the previous
entry. **22 offer cards, 10 accepts / 12 declines** (DoorDash's own dash counter is the oracle — see
Bug 1). Uber Offline throughout; one pause (hand-resumed), no force-stop, no unassign, no return
order, one stacked job (…-674); **one process for the whole day**, `restarts=0`,
`mappingFailures=0`, `notifListenerDisconnects=0`, **zero ERROR lines**, no `RecognitionHealth` /
`LocaleBoundary` / `TtsHealth` notice. 715 capture envelopes; log coverage continuous 09-20 09:03:09 →
22:02:13 (5 rotations + `app.log`). Pull dir `~/dashbuddy/logs/2026/09/21`, device purged after
verification; the phone was re-installed on `71bacc8c` (the PR #1118 merge) afterwards. Desk report:
`~/dashbuddy/logs/2026/09/21/DESK-ANALYSIS.md`.

### Bugs

1. **A real accept dies on ONE intermediate `waiting_for_offer` frame — four of ten accepts lost, and
   a phantom accept minted a job from an offer that was never taken.** DoorDash's own counters are the
   ground truth for the day: the receipt sheets read *Offers accepted 4 out of 5* (12:36:50) → *5 out
   of 9* → *6 out of 11* → *7 out of 12*, and the dash summary closes on **8 out of 16** ($215.12,
   7 hr 31 min). Session 661 therefore had exactly **8 accepts**, and all eight are independently
   corroborated: each maps 1:1 onto a `DELIVERY_COMPLETED` row and onto one shopping list whose
   `PICKUP_CONFIRMED.itemsShopped` (18/24/22/24/27/34/16/32) equals that card's `Shop for N items`.
   The machine attached **4** of them. What it did instead:

   | offer | $ | truth | machine |
   |---|---|---|---|
   | `0b2f12bc` | 27.84 | accept | **lost** — job …-668 minted with `jobOfferHashes=[]` |
   | `f54e4b64` | 11.50 | **not** accepted | **phantom** — minted job …-674 + a dropoff placeholder |
   | `137a3054` | 20.50 | accept (mid-job add-on) | **lost** — `OFFER_TIMEOUT`, `acceptedOffers` never grew |
   | `1c58c4aa` | 43.21 | accept | **lost** — job …-682 minted with `jobOfferHashes=[]` |
   | `75704d33` | 31.37 | accept (session 686) | **lost** — job …-687 minted with `jobOfferHashes=[]` |

   *Hypothesis for the mechanism:* `OfferLifecycle.resolveOnLeave` mints the accept survivor only on
   the ONE observation that leaves offer-presentation, and only when that observation's flow is a task
   flow. On 8.98.5 the post-card sequence is sometimes `offer_popup → waiting_for_offer →
   pickup_pre_arrival` instead of `offer_popup → pickup_pre_arrival`; the transient
   `waiting_for_offer` frame is then the leave edge, its destination is not a task flow, the pending
   offer is resolved away, and the pickup surface arriving a fraction of a second later finds no
   survivor to mint. The measured lag between `OFFER_TIMEOUT` and the following `PICKUP_NAV_STARTED`
   separates the two populations cleanly — **0 ms (the same observation) on all six attached offers**,
   against −22 / +53 / +66 / +931 ms on the four lost ones. Three log sequences show the intermediate
   frame directly: 11:05 `offer_popup → waiting_for_offer → Offer Timed Out! → pickup_pre_arrival`;
   15:36 `offer_popup → UNKNOWN → waiting_for_offer → pickup_pre_arrival`; 20:17 `offer_popup →
   UNKNOWN → side_nav_drawer → offer_popup → waiting_for_offer → pickup_pre_arrival`. The machine
   confesses the consequence itself on each lost job — `#997 offer-pay attribution degraded to
   JOB_POOLED … 0 offer(s) over 1 drop(s)` ×2 and `#691 … denominator=1 of 1 owed, ownOfferPay=null,
   jobOfferTotal=null` ×2. **Money:** $59.21 folded `payBasis NONE` / $0.00 across jobs …-668 and
   …-687 ($80.24 counting 09-19's job …-657, $21.03, the same shape). *A second hypothesis, separate
   from the first:* the phantom is the inverse edge — `f54e4b64` timed out at 12:38:43.732 and
   `PICKUP_NAV_STARTED job …-674` landed in the **same millisecond**, with no job running (job …-671
   had closed at 12:36:53), so a leave-edge that lands on a task flow appears to be accepted whether
   or not anyone pressed Accept. Both would need a `SessionReplay` of the 11:04 / 12:38 / 13:51 slices
   to confirm. Neither shape is one PR #1118 covers: its `!offer.returnFlow.isTaskFlow()` guard fires
   on the return flow, and all three mid-job offers here were logged `returnFlow=Idle` (R0 had fallen
   back to `waiting_for_offer` between the drop and the card).
   - **Status:** Open (#1119).

2. **The #1033 layer-2 re-price applied a single-drop receipt to every drop of the job and destroyed
   $28.51.** Job …-674 is the day's one stacked job, and its two drops were two different orders:
   drop 1 shopped 27 items and completed at 13:51:04 against its **own** receipt ($28.51, folded
   `DROP_SHARE $28.51`, `Saved: $28.51`); drop 2 shopped 34 items and completed at 14:40:39 against
   **its** own receipt ($20.50), whose expanded sheet says in DoorDash's own words *"This offer …
   $20.50"* — one drop. At 14:40:39 the log reads `#1033 receipt re-price: job …-674, **2 drop(s)**,
   receipt 2050¢`, and two `DELIVERY_RECEIPT_REPRICE` events set **both** drops to $10.25. The job
   earned $49.01 and is recorded at $20.50. `receiptRepricedAt` on row 2891 is the **first non-null in
   the corpus** — the first time layer 2 has ever applied rather than no-op'd, and the first
   application was wrong. *Hypothesis:* a `ReceiptCoverage` failure of exactly the class #1073 names —
   "a receipt speaks only for the drops it described when read" — where the coverage intersection did
   not exclude a sibling drop that had already folded its own receipt. Worth noting that the
   playbook's `rows_total == receipt_total` guard **passes** here (20.50 == 20.50) because it compares
   against the LAST receipt; the invariant is blind to a job whose earlier receipt was superseded.
   - **Status:** Open (#1120). (Moved here from the checklist — the #1033 layer-2 item was found
     **BROKEN** on this pull and is deleted from `Next field test`.)

3. **PLEDGE — the 8.98.5 dropoff customer-detail sheet falls to UNKNOWN on its un-anchored second
   frame and ships everything raw.** `dropoff_pre_arrival` claimed and correctly masked the sheet's
   FIRST frame at 15:09:16 (`Deliver to [redacted:…]` present); 17 s later the same sheet re-rendered
   **without** the `Deliver to` header, no rule matched, and it went to disk whole:
   `UNKNOWN/2026-09-20_15-09-33-393__…510649.json` (plus its twin `…-614__…9775b7.json` and a partial
   third at 15:27:58) carries a street line, a City/ST/ZIP line, an `Apt` unit number, a quoted
   customer instruction body containing a **gate code, twice**, and the customer's name in the bottom
   bar. Every node on that frame is id-less, so `ID_MARKERS` could not help; `CustomerTextMarkers`
   could not help (no `Deliver to` / `Delivery for` prefix survived); and the name is rendered
   **ALL-CAPS** as a first name + last initial, a form `FIRST_LAST_INITIAL_PATTERN` does not match.
   **This is a recurrence of the #1116 / #985 class** — the same "rule claims the first beat, the
   fully-inflated second beat drops the anchor and ships raw" failure, on a different sheet one pull
   later. *(The bare 3-digit number also on that sheet is DoorDash chrome — the same glyph appears
   beside `Customer tips` in `delivery_summary_expanded`. Not a code.)* The files are purged from the
   pull and the device was already purged, so no copy remains.
   - **Status:** Open (#1122).

4. **PLEDGE — `dropoff_pre_arrival`'s redact misses the bottom-bar customer-name node.**
   `dropoff_pre_arrival/2026-09-20_12-35-05-218__…8cc2d1.json` is a **recognized** envelope whose
   every other PII slot grades correctly (`Deliver to [redacted:a3a3]`, two more hashed values, three
   plain `[redacted]`) — and whose bottom-bar `TextView` carries the customer's first name + last
   initial **raw**. The identical structural slot is masked `[redacted:fa2f]` in
   `dropoff_workflow_sheet` and held the ALL-CAPS name of Bug 3 in the UNKNOWN frame, so the node
   exists across all three surfaces and only this rule's `redact` omits it. It is the only one of the
   14 `dropoff_pre_arrival` envelopes in which that node rendered. Per CLAUDE.md a recognized rule
   that forgets to redact has **no backstop**, which is why this is the same severity as Bug 3 despite
   being one node. Purged.
   - **Status:** Open (#1123).

5. **The mid-job ADD-ON offer card is not recognized on 8.98.5 — the dasher was shown +$10.50 and the
   app was blind to it.**
   `side_nav_drawer/2026-09-20_15-51-37-652__doordash__accessibility.window__side_nav_drawer__7f17fd.json`
   — two H-E-B store legs with their shop counts, `Customer dropoff` ×2, `Accept`, `Decline`, `0:45`,
   and the two lines that break it: the money is a **delta** (`+$10.50`, not `$10.50`) and the route
   summary is a **delta** (`+1 stop (1.5 mi) • +1 min`, not the `N stops (N mi) • N min` shape
   `offer_popup` requires). The frame therefore fell through to `side_nav_drawer`, and the dev got the
   `New Order: Go to H-E-B and 1 other store` push at 15:51:36, was shown a two-store add-on worth
   +$10.50 on top of a live job, and declined it blind at 15:51:52 with **no `OFFER_RECEIVED`, no
   evaluation, no bubble and no voice**. State-machine impact: none — no pending offer was ever
   created — so this is a **blindness** defect, not a corruption one. It is the only `+$` / `+N stop`
   card in the pull, and the only one of 41 `side_nav_drawer` captures that contains offer content.
   - **Status:** Open (#1121).

### Verification (desk)

1. **Money: $246.49 reported, $157.77 recorded, $88.72 (36 %) unattributed — and every dollar of the
   gap is Bug 1 or Bug 2.**

   | gap | $ | cause |
   |---|---|---|
   | job …-668 drop | 27.84 | accepted offer never attached → `payBasis NONE` (Bug 1) |
   | job …-674, 2 drops | 28.51 | the 2nd receipt re-priced BOTH drops (Bug 2) |
   | job …-687 drop | 31.37 | accepted offer never attached → `payBasis NONE` (Bug 1) |
   | residual, session 661 | 1.00 | unmodelled (peak pay / challenge) |

   Counting 09-19's session 656 (job …-657, the same lost-accept shape) the four-session total is
   $267.52 reported / $157.77 recorded. payBasis mix for 09-20: `DROP_SHARE` 5 rows $104.56 ·
   `OFFER_PAY` 2 rows $53.21 · **`NONE` 3 rows $0.00**. Invariants that DO hold:
   `Σ realizedMiles ≤ (lastOdometer − startOdometer)` on every session (largest gap 0.32 mi), **zero**
   negative `realizedMinutes`, and `rows_total == receipt_total` on all four receipt-priced jobs —
   the last of which is itself misleading here (Bug 2).

2. **(A) #1114 / PR #1115 — the Compose offer card is CONFIRMED (1/2), cleanly, with one new gap.**
   **22 offer cards → 22 `OFFER_RECEIVED` → 22 `offer_records` rows → 22 TTS utterances → 22
   screenshots → 22 chat cards → 22 resolutions**, a perfect 1:1 against two whole blank days on the
   previous pull. **Parse quality: zero nulls** — every row carries `payAmount`, `distanceMiles`,
   `itemCount`, `merchantName`, `estTimeMinutes`, `score`, `action` and `quality`; no blank store and
   no ghost merchant (`2 stops (…)` never leaked into `merchantName`); 21 rows `H-E-B` and 1
   `Taco Bell` (the only non-shop offer, `itemCount 0` — correct).
   `parseShortfall{doordash.screen.offer_popup…}` is **absent entirely**, the only shortfall entries
   being the documented benign baseline (`waiting_for_offer=35`, `pickup_shopping=6`,
   `dash_along_the_way=5`). **Both decline binds are back at 0** —
   `bindShortfall{…offer_popup#declineButton}` and `{…offer_popup_confirm_decline#confirmDeclineButton}`
   were 2 and 25 on the previous pull; 8 `Single verified candidate for decline_offer on doordash
   [doordash.screen.offer_popup] — clicking it` lines fired and the confirm sheet renders a real
   `Decline offer` node again on 8.98.5. The **`New Delivery!` push logs `NEW_ORDER` again** — 23
   `Rule LOG [NOTIFICATION_RECEIVED]: NEW_ORDER`, 4 `notification/new_order/` envelopes, on the new
   `…nexus-new-offer-no-sound-haptics` channel. Offer-shaped `UNKNOWN` frames: 4, all half-renders of
   a card recognized on an adjacent frame (the permitted #595 family);
   `accept_decline_footer_container` appears in **zero** UNKNOWN frames. The one gap is Bug 5 — the
   add-on card's delta shapes.

3. **#1102 CONFIRMED a second time — RETIRED at 2/2 (moved here from the checklist).** **5 receipts
   presented, 5 expanded** — `Performing expand_earnings` ×5 and `Single verified candidate …
   clicking it` ×5 — with **zero** `Could not find any live node for: expand_earnings` this time (the
   two mid-slide failures of the previous pull were the shape the fix rescues; here the tap landed
   first time on every receipt). The two `Throttled action` lines both land ~500 ms **after** a tap
   that had already succeeded, which is precisely the restored behaviour: a landed tap keeps its
   stamp, a failed one does not. 5 of the 8 delivery rows folded `DROP_SHARE`; the 3 `NONE` rows are
   Bug 1, not this item.

4. **#987 CONFIRMED a second time — RETIRED at 2/2 (moved here from the checklist).** Both
   `notification/earnings_deposit/` envelopes read `{"title": "[redacted]", "text": "Your Dasher
   earnings for [redacted]", "tickerText": …, "bigText": …}` — **plain, no hex, all four flat
   fields**. The playbook's dasher-banking sweep returns **0** on all three grep families tree-wide.

5. **#1063 — sixth consecutive clean desk pass.** 22 → 22 → 22 → 22 → 22 (cards → `OFFER_RECEIVED` →
   utterances → screenshots → resolutions), zero `Replaced by new offer` and zero `Superseded by
   direct offer`, `parseShortfall{offer_popup}` absent. Dev-eyes half still open.

6. **#1058 — PARTIAL PASS with a regression.** The two `dropoff_workflow_sheet` envelopes are graded
   correctly (hashed name, **plain** `[redacted]` for address, unit and note) — that half holds on
   8.98.5. But the adjacent-positive half that has carried this item for five pulls **broke**:
   `dropoff_pre_arrival` shipped a raw customer name in its bottom bar (Bug 4), and the same sheet's
   un-anchored second frame shipped everything (Bug 3). The alcohol arrival card (leak A of the
   original #1058) still has not rendered.

7. **#1104 / PR #1118 — UNEXERCISED, and this pull is its baseline.** The device build predates
   #1118, so all 22 offers logged `OFFER_TIMEOUT` against a ground truth of 10 accepts + 12 declines
   (the 12 declines each carry a `Denied confirm_decline` WARN or an `OfferActionReceiver:
   decline_offer` line — i.e. they reached the "are you sure" sheet). The next pull, on `71bacc8c`,
   is the first that can measure it. Bug 1 names a shape #1118 will **not** convert: an accept whose
   leave edge is a transient `waiting_for_offer` frame never reaches the transition #1118 reads.

8. **#1103 UNEXERCISED, clean negative.** `settle gate: receipt running-total read refused` = **0**
   hits — the causing shape did not recur and there was no *false* refusal either. Both
   `early_offline` zero-delivery sessions (660, 685) report NULL `reportedEarnings`; no
   `Delivery - <amt>.png` / `DeliveryBreakdown - <amt>.png` pair straddles a dash boundary; no
   `DELIVERY_RECEIPT_REPRICE` is attributed to a following session.

9. **#1054 UNEXERCISED for a SIXTH consecutive pull, both items, baseline healthy.** Three summary
   ends at 2.502 / 2.504 / 2.504 s and two early-offline ends at 10.003 s; `restarts=0` and no
   `Recovery re-armed` (correctly — one process all day, nothing to restore). The day's one pause
   (11:52:50, `remainingText 34:58`) was hand-resumed two seconds later and committed by
   `MODE_RESUME_COMMIT` at 11:53:06 → `Session grace resume`: the ordinary path, not the force-stop
   exercise. Only the two on-dash exercises can move these.

10. **#1107 UNEXERCISED (second pull running).** The `dropoff_step_instructions/` folder is **absent**
    — the 8.97.8 "Drop off steps" wrapper page did not render at all on 8.98.5 this dash, so neither
    the recognition half nor the `plainMask` half was touched.

11. **#1059 UNEXERCISED for a sixth pull, over-match check passing a sixth time.** The four ids return
    **0** tree-wide and none of the three new arms fired. 175 `pickup_shopping` + 70 `shopping_item`
    Red-Card shop frames produced **zero** `sensitive.red_card` drops. Pre-existing arms healthy:
    `dasher_direct` ×73, `catchall` ×7, `balance` ×5, `crimson_balance` ×2.

12. **#1108 CLEAN, not a confirmation.** `Σ realizedMiles ≤ span` on every session, zero negative
    `realizedMinutes`, and the one stacked job (…-674) shows legs 7.68 + 6.22 = 13.90 mi, both
    non-zero — the causing shape simply did not recur.

13. **#996/#997 degraded exactly twice, and both degrades are the machine reporting Bug 1 honestly.**
    2 × `#997 offer-pay attribution degraded to JOB_POOLED … 0 offer(s) over 1 drop(s)` + 2 × `#691 …
    denominator=1 of 1 owed, ownOfferPay=null, jobOfferTotal=null`, all four on the lost-accept jobs
    …-668 and …-687. Shapes (b) sub-pooled and (c) consolidated-customer remain unseen.

14. **WARN / ERROR census: 0 ERROR lines, `restarts=0`, one process for the whole day.** The census:
    13 `Timer Expired: GRACE_COMMIT` (designed commit path), **12 `Denied confirm_decline … (fail
    closed)`** (Open questions 3), 5 + 1 + 1 + 1 `Capture scrubbed: UNKNOWN screen / notification /
    click hit marker`, 2 `#691 … unsplit` (Verification 13), 2 + 1 `Capture backstop: UNKNOWN screen
    carried customer PII` (`nodeId=user_name` ×2, one text marker), 3 known-benign `ParseHealth`
    parse-shortfall lines (`waiting_for_offer` / `pickup_shopping` / `dash_along_the_way`), 1
    `delivery_summary_collapsed optional bind 'expandButton' resolved no node (#1093)`, 1 `Odometer
    fix rejected (POOR_ACCURACY) delta=26m dt=3580ms`, 1 `Timer Expired: MODE_RESUME_COMMIT` (the
    11:52 pause). `unknownScrubbed=8`, `unknownCustomerScrubs=3`.

15. **UNKNOWN census: `unknownDropped=213`; 82 window + 99 click + 5 notification envelopes written.**
    Heads: 26 text-free Compose loading/transition skeletons, 8 single-string Compose chrome frames,
    6 side-menu / top-bar strips, **4 offer-card partial renders** (`Decline` + route line, no
    `Accept` / disclaimer — the permitted #595 family), 4 post-decline "This dash so far" sheets, 4
    nav-map chrome, 3 offer cards composited over the side-nav drawer, **3 dropoff customer-detail
    sheets (Bug 3)**, 3 dash-menu / timeline sheets, 3 support-chat surveys, 3 drop-off photo steps, 3
    camera sheets, 3 weekly-earnings screens, then one-offs. **Clicks: 99 UNKNOWN against 26
    classified** (`arrived_at_store` 9, `take_photo` 10, `open_chat` 7, `resume_dash` 1) and **zero**
    classified `accept_offer` / `decline_offer` / `confirm_decline` — consistent with the standing
    finding that a human tap on a Compose control emits no `TYPE_VIEW_CLICKED`. Notifications: 4
    `new_order`, 13 `customer_message`, 2 `earnings_deposit`, 1 each `marketing_promo` /
    `demand_nudge` / `dash_status_ongoing`, 5 UNKNOWN (marketing / demand nudge / a dash-paused push
    carrying the dasher's own first name, which is allowed).

16. **PII sweep of all 715 envelopes: two real leaks, both filed, both purged; everything else clean.**
    The sweep walked `text` / `desc` / `stateDescription` / `hintText` and the flat notification
    fields for two-capitalised-word names, the `[A-Z][a-z]+\s{1,4}[A-Z]\.` shape, `Pickup for` /
    `Deliver to` / `Delivery for` / `For <Name>`, `pin|gate code|entry code \d{3,}`, street addresses,
    `Apt|Unit|Suite \d+`, `City, ST 12345` and `Customer Notes`. Dasher-banking greps: **0 / 0 / 0**
    tree-wide. The two findings are Bug 3 and Bug 4. Clean everywhere else:
    `dropoff_workflow_sheet`, `timeline` (both envelopes fully masked — the #1116 shape did **not**
    reproduce on `timeline` itself, the class simply moved to a different sheet), `earnings_deposit`
    and `customer_message` all grade correctly; the 99 click envelopes are clean (the only oddity is
    Google-Nav `primaryManeuverText` road names without house numbers, which are not addresses); the 5
    UNKNOWN notifications are clean. The live backstop **fired 3×** — `Capture backstop: UNKNOWN
    screen carried customer PII` (two on `nodeId=user_name`, one on a text marker) — plus 8 marker
    scrubs, which is the privacy layer working on frames no rule claimed.

17. **Not exercised:** #998 (no refused or returned delivery); **#301** (the dev did reach
    `pickup_select_issue` → `pickup_resolution_options` at 12:38:53 but took **Contact support**, not
    "Unassign order" — zero `TASK_UNASSIGNED`, zero phantom `PICKUP_CONFIRMED`, so not a test of
    #301); #1034 (`grep -c '\$-' shareable.log` = 0, no window went negative); #967; every Uber item;
    every dev-eyes UI item. **#991 no-regression** — 22/22 `speak()` succeeded, zero `-1`, zero
    re-init, zero notice.

### Open questions

1. **PR #1118's guard is correct and does not cover any shape this dash produced.** Three offers were
   presented while `activeJob != null`: `0b2f12bc` (11:04:45, job …-665 still open — a real accept
   that was lost), `d455aa04` (13:51:04, `returnFlow=PostTask` — a decline the machine handled
   perfectly: no phantom, no placeholder) and `137a3054` (13:51:24, job …-674 still open — a real
   add-on accept that was lost). #1118's `!offer.returnFlow.isTaskFlow()` fires on the offer's
   *return* flow, and both mid-job accepts were logged `returnFlow=Idle`; the one phantom
   (`f54e4b64`) also had `returnFlow=Idle` and **no job running at all**. *One possibility* is that
   R0 falling back to `waiting_for_offer` between a drop and the next card is what makes
   `returnFlow=Idle` on a genuinely mid-job offer — which, if it holds, is the same root as Bug 1 and
   would mean the return-flow guard and the leave-edge tolerance are two views of one problem.
   Confirming needs a replay, not a log read.

2. **The playbook's `rows_total == receipt_total` money invariant is blind to a superseded receipt.**
   It compares each job's rows against the LAST figure the machine stated for that job, so job …-674
   passes (20.50 == 20.50) while $28.51 is missing. Worth deciding whether the check should compare
   against the Σ of *distinct* receipts seen for a job, or whether `ReceiptCoverage` should be
   asserted directly — either way the desk-validation playbook's step needs a note so the next pull
   does not read a green invariant as a clean job.

3. **The confirm-decline capability is UNGRANTED on this install** — 12 × `Denied confirm_decline —
   no granted capability for rule 'doordash.screen.offer_popup_confirm_decline' (fail closed)`. Not a
   defect (the #843 no-auto-grant posture working as designed, its tenth corroboration), but it means
   #577 quick-decline auto-confirm was off for the whole day and stays off until the consent prompt is
   answered. **Dev action:** re-Allow it in the consent prompt or Settings → Data & Privacy →
   Automation & Consent — the phone was re-installed on `71bacc8c` afterwards, so the prompt is
   waiting with this row (and #1115's Compose accept row) undecided.

4. **`FIRST_LAST_INITIAL_PATTERN` does not match an ALL-CAPS name.** Bug 3's bottom-bar name reached
   disk partly because the intake name shape expects `Firstname I.` casing. *Hypothesis:* an ALL-CAPS
   arm on the name shape is a rules-independent improvement that would have caught this frame even
   with no rule claiming it — but it widens a scrub layer, and per CLAUDE.md widening a scrub layer
   must never move a classification, so it wants its own look rather than being folded into the rule
   fix.
...
## 2026-09-16 → 2026-09-19 (desk analysis of the 09-20 pull — DoorDash's new Compose offer card blanked two whole days of offers; #1078 caught three more dash-end drops; a timeline sheet's SECOND frame shipped an address raw)

**Date:** 2026-09-16 → 2026-09-19 · **Platform(s) tested:** DoorDash (app **8.97.8**, every envelope —
unchanged across the 09-18 offer-card rollout, so that was a server-side UI flip, not an app update) ·
**Branch under test:** `master` at `1bb09795` (the PR #1112 merge; read off the logs — build
`0.230.0+1bb09795` on the startup line ×4 processes and on the `app=` head of all 112 `PipelineStats`
lines, per PR #1066). **First field run of #1110 / #1111 / #1112.** · **Field conditions:** fifteen
DoorDash dashes over four days — 09-16: 578 ($64.55, 2 drops), 585 ($20.25), 589 ($14.50); 09-17: 593
($47.29, 3), 601 ($76.30, 3); 09-18: 611 ($48.30, 2), 618 ($23.71); 09-19: 622 ($18.51, 2), 630
($156.34, 5), 646 (`early_offline`), 647 ($61.55, 2), 654 + 655 (`early_offline`), 656 ($21.03), 660
(`early_offline`). **23 drops, $552.33 reported, 241.75 mi of odometer span.** 23 offers were
recognized on 09-16/17 (4 acc + 7 dec + 1 timeout, then 5 acc + 6 dec + 0 timeout) and **zero** on
09-18/19 — the Compose rollout, Bug 1. Uber Offline throughout; no pause, no force-stop, no unassign,
no return order, one stacked job; two app starts (09-15 23:51, 09-17 23:56) and **zero crash
recoveries**. 1 440 captures (1 105 window / 314 click / 21 notification; 159 UNKNOWN windows, 166
UNKNOWN clicks, 21 UNKNOWN notifications), **112 WARN / 0 ERROR**, `restarts=0` and
`notifListenerDisconnects=0` across the window, no `RecognitionHealth` / `LocaleBoundary` /
`TtsHealth` notice, `shareable.log` PII-free. One capability granted (the #1093 receipt-expand);
`confirm_decline` still denied fail-closed 23×. Log coverage continuous 09-15 20:35:56 → 09-20
07:30:13 across 13 rotations + `app.log`, no gaps. Pull dir `~/dashbuddy/logs/2026/09/20`, device
purged after verification. Desk report: `~/dashbuddy/logs/2026/09/20/DESK-ANALYSIS.md`; Astra's
independent preliminary review of the same pull: `~/codex/dashbuddy/2026-09-20-preliminary/REVIEW.md`.

### Bugs

1. **DoorDash moved the offer card to Compose on 09-18 and DashBuddy recorded ZERO offers for two
   dashing days.** 13 offer-shaped frames fell to `UNKNOWN/` and another 23 were misfiled as
   `side_nav_drawer/` (that rule's `side_nav_content_container` + ratings/promos/preferences anchors
   coexist with the offer content in the captured tree); `app_events` holds **no `OFFER_*` row of any
   type and no `offer_records` row on either day**, against 12/12 on 09-16 and 11/11 on 09-17. All 36
   frames lack `accept_button`, `accept_decline_footer_container` and `display_name`, which the
   `offer_popup` rule requires. First frame: `UNKNOWN/2026-09-18_14-46-44-021…4ff4dc.json` (Dollar
   Tree, $17.75, 7.6 mi); a clean live one: `UNKNOWN/2026-09-19_11-42-11-094…b7abab.json` (H-E-B,
   $14.75, 8.5 mi, Accept + countdown); last old-layout recognized card
   `offer_popup/2026-09-17_13-44-02-213…c7aded.json`. **Money consequence:** all six `payBasis = NONE`
   rows of the pull (Verification 1) are on these two days — with no `OfferEvaluation` to fall back on,
   a drop is priced only if a receipt is read. Everything else in the lifecycle held without offers:
   jobs were still minted from the pickup surface (click-less accept) and each ran the full
   `PICKUP_NAV_STARTED → … → DELIVERY_COMPLETED` chain, 23/23 rows carry `storeName` + `storeKey`, zero
   ghost offers, zero `JOB_ACCEPT_MISMATCH`. The **push** moved the same day too: the offer
   notification's `channelId` flipped from `dasher-notification-channel-new-order-v2` (11 envelopes,
   09-16/17) to `dasher-notification-channel-nexus-new-offer-no-sound-haptics` (16 envelopes,
   09-18/19), so every offer push fell UNKNOWN and `NOTIFICATION_RECEIVED: NEW_ORDER` stopped firing —
   the `nexus` token is the same rollout. *Hypothesis:* a server-side UI flip inside 8.97.8 (the
   evidence brackets it between 09-17 13:44 and 09-18 14:46 but does not establish the rollout
   mechanism).
   - **Status:** Shipped in #1115 (2026-09-20, merged at `aab8d960`) — a second `offer_popup` branch
     for the Compose card, the drawer rule taught to reject it, and both channel ids accepted in
     `notifications.json5`. **Unvalidated in the field:** the checklist already carries #1115's own
     watch item (added by that PR), including whether an ACCEPT still mints the job and whether a
     DECLINE lands as a decline or as a timeout.

2. **PLEDGE — the timeline order-detail sheet escapes #985 on its SECOND frame and shipped a customer
   address and instruction note raw to disk.** The sheet renders in two beats and only the first is
   claimed:
   ...
   — raw street line, raw city/ST/ZIP, and a raw quoted instruction body carrying a unit number; the
   name alone was caught by the `user_name` ID-marker backstop. The #985 rule requires **both**
   `Copy address` (text) and `Close sheet` (contentDescription), and the fully-inflated frame 225 ms
   later no longer carries `Copy address`. *Hypothesis:* the same class as #1058 leak B and #803 — an
   id-less customer-instruction body on a sheet a structural anchor failed to claim; either #985's
   second anchor is too tight for the inflated frame, or the address/note shapes belong on the
   UNKNOWN-path backstop for this sheet. This file was the **only** PII hit in a sweep of all 1 440
   envelopes (Verification 10); it has been purged from the pull, and the device was already purged,
   so no copy remains.
   - **Status:** Open (#1116).

3. **The offer confirm-decline sheet has no button label at all on 8.97.8 — the decline anchor is
   fully rotted, and the #1104 `screenIs` widening cannot have fired.**
   `offer_popup_confirm_decline/2026-09-16_18-40-14-504…83204c.json` carries only copy lines (`Are you
   sure you want to decline this offer?`, `Does not lower acceptance rate`, `30%`, `A high acceptance
   rate is required…`, `Pause offers from this store`, `Thanks to your Gold status`) — **no `Decline
   offer` node**. Consequences, all observed: final `PipelineStats`
   `bindShortfall{doordash.screen.offer_popup_confirm_decline#confirmDeclineButton=25}` (it was 3 WARNs
   on the 09-15 pull), an earlier process line carrying `offer_popup#declineButton=2`, two
   `ParseHealth … optional bind 'confirmDeclineButton' resolved no node — target anchor rot?` WARNs,
   one `No 'declineButton' target bound for doordash — decline_offer unavailable`, and **zero** clicks
   classified `confirm_decline` in 1 440 envelopes. *Hypothesis:* this, not the click/window race, is
   the live remaining mechanism behind #1104's fielded shape — and it may not be repairable from the
   click side at all: **#1115's new Compose `confirmDeclineButton` bind restores the TARGET, but a
   human tap on a Compose control emits no `TYPE_VIEW_CLICKED` at all**, so no click envelope can
   arrive to be classified (research note
   `~/dashbuddy/research/compose-click-capture-2026-09-20.md` §0–§1: Compose emits that event only for
   an accessibility-performed `ACTION_CLICK`, which is why DashBuddy's own automation taps still
   produce envelopes while the dasher's do not; the note is linked from #1114's comment). If that
   holds, a decline would have to be inferred from the semantics the tap causes rather than from the
   tap itself — unconfirmed, and no issue is filed for it yet.
   - **Status:** Partially closed — the bind half shipped in #1115 (2026-09-20); the "can a human tap
     ever be observed" half is open and unfiled. The decline-bound checklist item stays live carrying
     this evidence.

4. **A new unrecognised handoff-photo instruction surface.**
   `UNKNOWN/2026-09-18_15-36-07-896…0bf650.json` — `description_item: 'Take a photo of the handoff
   location'`, `first_caption: 'Take a door photo\nInclude the apt/suite number if there is one'`,
   plus `second_caption`, `Continue`, `Contact support`, under `drop_off_workflow_host_fragment`. No
   PII on this render. *Hypothesis:* by the #1107 / #1058 argument this is exactly the kind of wrapper
   a customer note can inflate into, so it is a recognition gap today that could become a leak later.
   - **Status:** Open (logged, low — not filed).

### Verification (desk)

1. **Money: $442.94 of $552.33 attributed; the $109.39 gap is six `payBasis = NONE` rows and is
   mechanically Bug 1.** Per-session `Σ realizedPay` equals `reportedEarnings` **to the cent on nine of
   the ten sessions that priced**, and the five `early_offline` zero-delivery sessions all report NULL.
   Σ pay never exceeds reported anywhere (the #701 over-attribution check passes). Attribution splits
   `DROP_SHARE` 11 rows / $307.07 + `OFFER_PAY` 6 rows / $135.87; every one of the six unattributed
   rows is on 09-18/19 and each carries its own WARN pair, e.g. `09-18 15:37:39.797 DEBUG/StateMachine:
   #997 offer-pay attribution degraded to JOB_POOLED: job job-doordash-1789760804545-612, 0 offer(s)
   over 1 drop(s)` → `WARN/StateMachine: #691 offer-pay estimate eligible but unsplit … these dollars
   ride the unattributed bucket`. Of the 13 drops on the offer-less days, 7 landed a collapsed receipt
   and priced `DROP_SHARE`; 6 never saw a receipt frame at all (mid-route or dash-end completions) and
   fell through to `NONE`.

2. **#1078 + #1095 CONFIRMED a second time — RETIRED at 2/2 (moved here from the checklist).** Three
   drops were recorded at the `DASH_STOP` edge that the pre-#1100 build would have lost silently:

   | seq | when | `completedAt` stamped | `DASH_STOP` `occurredAt` | gap |
   |---|---|---|---|---|
   | 2730 | 09-18 | 17:12:43 | 17:12:54 | 11 s |
   | 2758 | 09-19 | 09:37:21 | 09:47:57 | **10 m 36 s** |
   | 2832 | 09-19 | 20:36:19 | 20:36:30 | 11 s |

   In each, the `DELIVERY_CONFIRMED` + `DELIVERY_COMPLETED` pair sequences immediately **after** the
   `DASH_STOP` row at the same `occurredAt`, with `completedAt` stamped at the absorbed retire's arm
   instant — the honored-retire signature. Seq 2758 is the strongest evidence: the drop confirmed at
   09:37:31, then the phone went idle for ten minutes so **no frame ever arrived to lapse the retire's
   10 s grace** (lazy expiry); the `SESSION_END` at 09:47:57 absorbed the still-live retire and
   `endSession` honoured it at its matured deadline. Pre-#1100 that dash records nothing. Across the
   pull: 23 delivered drops → 23 `DELIVERY_COMPLETED`, `Σ realizedPay == reportedEarnings` on every
   session that priced, **zero `JOB_ACCEPT_MISMATCH`** (so #1095's false-positive half passes a second
   time). The deliberate < 10 s End-Dash off the drop-off photo was never scripted, but the two 11 s
   gaps are that shape and the ten-minute idle exercises the absorption harder than the script would —
   the item is retired on that basis. One open question rides along (Open questions 1).

3. **#1102 CONFIRMED on its first field run (1/2).** `Performing expand_earnings` = 14,
   `Throttled action` = **0**, `Could not find any live node for: expand_earnings` = 2 — and both
   mid-slide failures were rescued by the retry *inside* what used to be the throttle window, which is
   precisely the fix:

   ```
   09-18 18:12:02.939 WARN/Effects: Could not find any live node for: expand_earnings … (id=null, text=null, bounds=…)
   09-18 18:12:03.465 INFO/Effects: Performing expand_earnings on doordash          ← +526 ms, NOT throttled
   09-19 12:19:53.486 WARN/Effects: Could not find any live node for: expand_earnings …
   09-19 12:19:54.293 INFO/Effects: Performing expand_earnings on doordash          ← +807 ms
   ```

   **12 distinct receipts presented, 12 expanded (100 %)**, against 6/11 on 09-13 and 16/18 on 09-15;
   collapsed → expanded latency 0.9–1.4 s on every one; 11 of the 12 produced a `DROP_SHARE` row (the
   twelfth is Open questions 2). Needs one more clean dash.

4. **#1103 UNEXERCISED, clean negative.** `settle gate: receipt running-total read refused` = **0
   hits** — the causing shape (a previous dash's receipt still up at `DASH_START`) did not recur, and
   there were no *false* refusals either. All five `early_offline` zero-delivery sessions report NULL
   `reportedEarnings`, including 646, which started 3 min 3 s after session 630 closed on $156.34. Six
   `SESSION_PAY_SETTLE` timers fired, each inside its own session and each committing that session's
   own figure; no duplicate `Delivery - <amt>.png` / `DeliveryBreakdown - <amt>.png` pair straddles a
   dash boundary; no `DELIVERY_RECEIPT_REPRICE` is attributed to a following session.

5. **#1104 IMPROVED, not closed — and the mechanism has moved.** One `OFFER_TIMEOUT` in 23 offers on
   09-16/17, against 9-of-12 misrecorded on the 09-15 pull. That one **was** a real decline (offer
   `0c4fc920`, 09-16 18:40): `Captured click: target=initial_decline` 18:40:12.880 → `UNKNOWN click —
   id=null text=null` 18:40:14.036 (the confirm tap) → `Captured screen:
   target=offer_popup_confirm_decline` 18:40:14.504 → `[Dispatch]: Offer Timed Out!` 18:40:15.104. But
   there are **zero clicks classified `confirm_decline` anywhere in the pull**, so PR #1110's
   `screenIs`-array widening cannot be shown to have fired at all — the anchor it would fire behind is
   dead (Bug 3).

6. **#1107 CONFIRMED on its recognition half; the redaction half is UNEXERCISED.** The 8.97.8 "Drop off
   steps" wrapper is live and claimed: **5 `dropoff_step_instructions` envelopes** (09-19), none in
   `UNKNOWN/` — first sighting of this folder. Every fielded frame carried only DoorDash's own step
   copy (`Leave it at the door`, `Verify correct order`, `Check you have the correct order from
   pick-up`, `Take photo of drop-off location`, `Handed order to customer`) with **no
   `description_text_view` node rendered on any of them**, so the `plainMask` on that node never had
   anything to mask. `step_title` / `step_description` correctly stay raw (app vocabulary, per the
   rule's own comment). The leak shape that produced #1107 needs an order that actually carries a
   customer instruction body.

7. **#987 CONFIRMED (1/2).** **11 of 11** `earnings_deposit` envelopes are fully masked, plain (no
   hex), all four fields: `{"title": "[redacted]", "text": "Your Dasher earnings for [redacted]",
   "tickerText": …, "bigText": …}`. The dasher's own figure and the #599 Crimson clause are both gone —
   the asymmetry logged as Bug 9 of the 09-15 entry (twelve unmasked instances across seven pulls) is
   closed on the recognized path. The playbook's dasher-banking sweep over the whole `captures/` tree
   returns **0** hits on all three grep families (`available balance|dasherdirect|crimson|routing
   number`, `transfer out|transfer in|cash out|instant pay`, `transfer +\$[0-9]`). First sighting since
   PR #1110 shipped; a checklist item now carries the second pass.

8. **#1058's workflow sheet EXERCISED for the first time, and clean.** `grep -c dropoff_workflow_sheet`
   = 2 (non-zero for the first time in five pulls), and the single `dropoff_workflow_sheet` envelope
   (09-17 10:59) is graded correctly: `[redacted:de0f]` for the name, **plain** `[redacted]` ×5 for the
   address lines / unit / note, `Leave it at the door` kept, merchant item descriptions kept. The
   adjacent positive holds — all 34 `dropoff_pre_arrival` envelopes mask correctly. Check 1's grep
   returns 7 UNKNOWN hits: 6 are the documented text-free loading skeletons and the 7th is Bug 4. The
   **alcohol** arrival card (leak A) did not render this window, so the item stays open for that half.

9. **#995 CONFIRMED on one frame.** `pickup_receipt_scan/2026-09-19_09-11-35-106…`: `Focus on
   [redacted:d67c]` / `Scan customer name` / `[redacted:d67c]` / `#12f24 · DoorDash` / `No receipt` —
   recognize-and-redact working as designed on 8.97.8.

10. **PII sweep of all 1 440 envelopes: one file, and it is Bug 2.** Walked `text` / `desc` /
    `stateDescription` / `hintText` and the flat notification fields for two-capitalised-word names,
    `[A-Z][a-z]+\s{1,4}[A-Z]\.`, `Pickup for` / `Deliver to` / `Delivery for` / `For <Name>`, `pin|gate
    code|entry code \d{3,}`, street addresses, `Apt|Unit|Suite \d+`, `City, ST 12345` and `Customer
    Notes`. Everything else found is allowed by design: merchant addresses raw (#886 —
    `pickup_navigation` ×14, `side_nav_drawer` ×13 incl. the Compose cards, 4 UNKNOWN Compose frames),
    `Customer Notes` ×8 in `shopping_item/` as the **label** only (`body_text_view: [redacted]`, fused
    `desc: Customer Notes: [redacted]`), and `Leave it at the door` as an instruction *title* on a card
    whose `user_name` / `address_line_1` / `address_line_2` are all `[redacted]`. Zero PII in any click
    or notification envelope; no dasher-name hits anywhere. The one offending file is purged.

11. **#1033 Layer 2 stays 1/2 (fifth consecutive pull without the deliberate > 8 s expansion).** Fired
    11×, no-op'd 11×, `receiptRepricedAt` NULL on all 23 rows, zero `DELIVERY_RECEIPT_REPRICE: no
    delivery row` WARN, and `rows_total == receipt_total` to the cent on all 11 receipt-priced jobs.
    Layer 1 is comfortable — every collapsed→expanded transition landed in 0.9–1.4 s. One possible new
    gap is in Open questions 2.

12. **#1054 UNEXERCISED for a FIFTH consecutive pull, both items.** Zero `DASH_PAUSED` / `DASH_RESUMED`
    / `MODE_RESUME_COMMIT` / `SESSION_PAUSED_SAFETY` in the window, no force-stop mid-grace, and no
    `Recovery re-armed` (correctly absent — neither of the two process starts restored a live grace).
    Baseline healthy: eleven summary ends at `occurredAt − endedAt` = 2.502–2.506 s, four early-offline
    ends at 10.002–10.004 s, no end drifted. Only the two on-dash exercises (background an offline
    screen; force-stop mid-pause) can move these.

13. **#1059 UNEXERCISED for a fifth pull, with the over-match check passing a fifth time.**
    `grep -rlE 'persona_container|personaComposeView|activate_physical_card_button|id_type_selector'`
    over the whole `captures/` tree returns **0**, and no `sensitive.selfie_verification` /
    `sensitive.red_card` / `sensitive.id_verification` drop fired. 269 `pickup_shopping` + 125
    `shopping_item` frames (many Red-Card shop orders) produced **zero** `sensitive.red_card` drops.
    Pre-existing arms healthy: `sensitive.dasher_direct` ×500, `sensitive.balance` ×25,
    `sensitive.crimson_balance` ×11, `sensitive.catchall` ×34.

14. **WARN / ERROR census: zero `ERROR` lines in the whole four-day window, 112 WARNs, `restarts=0`.**
    The census head: 35 `Timer Expired: GRACE_COMMIT` (the designed commit path), 23 `Denied
    confirm_decline … (fail closed)` (Open questions 3), 15 `Capture backstop: UNKNOWN screen carried
    customer PII (nodeId=user_name) — scrubbing node`, 6 `#691 … unsplit` (Verification 1), 6 + 4 + 2 +
    1 `Capture scrubbed: UNKNOWN notification / click / screen hit sensitive marker`, 3 `Odometer fix
    rejected (POOR_ACCURACY) … entering a rejection streak`, 2 + 1 anchor-rot `ParseHealth` lines
    (Bug 3), 2 + 2 `expand_earnings` resolution lines (both rescued, Verification 3), and the
    known-benign `waiting_for_offer` / `pickup_shopping` / `dash_along_the_way` parse-shortfall
    baselines. The 18 `Capture backstop` / `Capture scrubbed` WARNs are the privacy layer **working**
    on UNKNOWN frames — the 15-hit `user_name` cluster is the offer-less days' task cards falling to
    UNKNOWN and being scrubbed on the way to disk. Final `PipelineStats`: `forwarded=1650
    dupSuppressed=50963 unknownDropped=272 sensitiveDropped=276 noiseDropped=0 mappingFailures=0
    awaitingRulesDropped=0 unknownScrubbed=9 redactBackstopScrubs=0 unknownCustomerScrubs=15
    notifListenerConnects=1 Disconnects=0 restarts=0` — the #430 restart family and the #731 listener
    flap are both quiet.

15. **UNKNOWN census.** **Windows 159** (09-16: 38 · 09-17: 20 · 09-18: 45 · 09-19: 56): 22 side-nav /
    drawer chrome with no content, 17 text-free chrome frames, **13 Compose offer cards (Bug 1)**, 13
    Google-Nav sheet / maneuver chrome, 13 task-card variants with an address block, 12 text-free
    workflow-host loading skeletons, 7 id-less `Close sheet` bottom sheets (the timeline family of
    Bug 2), 6 legacy offer-card half-renders (the #595/#1063 family the check permits), 4
    shopping-item sub-sheets, 52 one-off dialogs — plus the 23 Compose offer frames misfiled under
    `side_nav_drawer/`. **Clicks 166** — all ordinary unmapped in-app buttons, none carrying PII
    (`Confirm` ×9, `Continue` ×9, `End dash` ×8, `Scan again` ×7, `Got it` ×7 …); **no Compose
    Accept/Decline tap appears in any click envelope**, which corroborates Bug 3's mechanism, and the
    09-19 growth (73 clicks vs 33 on the offer-rich 09-17) is shopping-flow chrome, not offer taps.
    **Notifications 21**, of which **18 are the offer push** on the new channel (Bug 1).

16. **Also clean:** #1108 — no recurrence (`Σ realizedMiles ≤ (lastOdometer − startOdometer)` on all 15
    sessions, largest gap 0.03 mi; zero negative `realizedMinutes`; the one stacked job, …-594 with two
    drops, shows legs 21.98 vs span 21.98); #1063's desk half a **fifth** time for 09-16/17 (23 offers
    → 23 `OFFER_RECEIVED` → 23 TTS utterances → 23 `Offer -` screenshots → 23 chat cards → 23
    resolutions, zero `Replaced by new offer` / `Superseded by direct offer`, and
    `parseShortfall{doordash.screen.offer_popup…}` absent entirely); #733/#159/#1000 store resolution
    (23/23 rows carry `storeName` + `storeKey`, zero `D6 join miss`, all 9 accepted offers carry
    `linkedJobId` **and** `storeKey`); #991 no-regression (23/23 `speak()` succeeded, zero `-1`, zero
    re-init, zero notice); #862 scrub visibility (zero `[scrubbed:` residuals and no raw
    merchant/customer text in the INFO+ `shareable.log`); #315 H5 (23 pickup dwells, 0.01–56.22 min —
    the 0.01 min row is Open questions 4); #996/#997 partially (6 `DELIVERY_COMPLETED` carry
    `PER_OFFER_STORE`; the 6 degrades are all `JOB_POOLED` with `0 offer(s)`, the #1114 artefact rather
    than a store-reconcile failure — shapes (b) and (c) are still unseen).

17. **Not exercised:** #998 (no refused or returned delivery); #301 (zero `TASK_UNASSIGNED`); #1034
    (`grep -c '\$-' shareable.log` = 0, and no window went negative); #967 (no Offline ratings browse);
    every Uber item; every dev-eyes UI item.

### Open questions

1. **Seq 2758 has `arrivedAt = NULL` and there is no `DELIVERY_ARRIVED` event for task …-629** (09-19
   shows 9 arrivals against 10 completions — this is the one), yet `absorbableRetire` is documented to
   require an ARRIVED dropoff. *Hypothesis:* either the `Task` carried an arrival the event stream
   never emitted, or a different path (the `endSession` close-out sweep) stamped this row. The outcome
   was correct either way, but which path it was matters, because #1095's lifted single-accept floor is
   keyed on the ARRIVED shape.

2. **The 09-17 12:01 drop is the shape #1033 Layer 2 exists for, and Layer 2 did not fire.** Seq 2676,
   job …-598: the completion committed on a receipt-less PostTask exit at 12:01:10.782
   (`WARN/Effects: Timer Expired: GRACE_COMMIT`), the collapsed receipt did not appear until
   12:01:21.465 — **10.7 s later** — the auto-tap expanded it at 12:01:21.985, and the screenshot pair
   reads `Delivery - 23.25` / `DeliveryBreakdown - 23.25`. No `DELIVERY_RECEIPT_REPRICE` was emitted
   for that job and the row kept `payBasis = OFFER_PAY $23.25`. **No money impact** — the offer
   estimate happened to equal the receipt to the cent. The obvious refusals do not obviously apply:
   `lastAcceptResolvedAt` for this platform was 11:02:05, well *before* `receiptSeenAt`, and the
   12:01:14→12:01:21 offer in the window was **declined**, which `OfferLifecycle` does not fold into
   `lastAcceptResolvedAt`. *One possibility* is that the expanded frame arrived after `DASH_STOP`'s own
   grace had taken the region out from under the anchor (`lastAnnouncedPostTaskTaskId`); confirming
   would need a `SessionReplay` of that slice.

3. **The confirm-decline capability is UNGRANTED on this install** — 23 `Denied confirm_decline — no
   granted capability for rule 'doordash.screen.offer_popup_confirm_decline' (fail closed)` WARNs. Not
   a defect (the #843 no-auto-grant posture working as designed, its ninth corroboration), but it means
   #577 quick-decline auto-confirm has been off for the whole window and stays off until the consent
   prompt is answered. Worth telling the dev before the next dash — the new Compose accept capability
   from #1115 will also arrive **undecided**, so the prompt will have more than one row.

4. **A 0.6-second pickup dwell** (0.01 min) sits at the bottom of a 23-row dwell distribution whose top
   is 56.22 min. No known consequence; worth one glance at which job it belongs to before the #588
   shop-rate model reads it as a sample.
...
## 2026-09-13 → 2026-09-15 (desk analysis of the 09-15 pull — #1078 caught a $35.50 drop with 0.4 s to spare; a return order walked off with $10.93; a gate code reached disk)

**Date:** 2026-09-13 → 2026-09-15 · **Platform(s) tested:** DoorDash (app **8.97.8**, all 1 427
envelopes — a new platform version; the corpus is 8.93.7/8.95.6/8.96.8-era) · **Branch under test:**
`master` at `8a5ea7c3` (post-#1100; read off the logs — `DashBuddy 0.230.0+8a5ea7c3 starting (built
2026-09-13T14:14:09.261Z)`, 63 `PipelineStats` lines all `app=0.230.0+8a5ea7c3`). **First field run of
#1078/#1095.** · **Field conditions:** fourteen DoorDash dashes over three days — 09-13: 492
(09:36–11:36 $58.27), 499 ($50.26), 507 ($60.67), 514 ($62.14); 09-14: 521 ($32.75), 529 ($62.50), 542
($32.78), 547 ($15.75), 551 ($77.26), 564 ($21.25); 09-15: 568 (`early_offline`, one accept unassigned
at the store), 572 ($18.05), 576 (`early_offline`, 11 offers in 9 min), 577 (a 4-second dash). Mostly
H-E-B shop-and-deliver plus McDonald's and Shipley Donuts; one alcohol delivery refused for an underage
ID and converted to a **return order**; one stacked 2-drop job; 82 offers (23 accepted / 47 declined /
12 timed out); Uber Offline throughout; no pause, no force-stop, one process for the whole window.
1 427 captures (172 UNKNOWN screens, 207 UNKNOWN clicks, 5 UNKNOWN notifications; `unknownDropped` 502
/ `forwarded` 3 150 ≈ 15.9 %), **185 WARN / 0 ERROR**, no `RecognitionHealth` / `LocaleBoundary` /
`TtsHealth` notice, `shareable.log` PII-free. One capability granted (the #1093 receipt-expand, carried
across the reinstall); `confirm_decline` still denied fail-closed 94×. Pull dir
`~/dashbuddy/logs/2026/09/15`.

### Bugs

1. **The "Drop off steps" wrapper shipped a customer's gate code, cross streets and a landmark raw to
   UNKNOWN capture.** Three envelopes (`UNKNOWN/2026-09-13_11-57-01-419…__b25762`, `…11-57-02-476…__93686e`,
   `…11-57-15-266…__01807f`), node `description_text_view` under
   `drop_off_step_instructions_activity_host_fragment`, page title `Drop off steps`, subtitle
   `Leave it at the door`. No rule, no backstop: the id is not in `ID_MARKERS` and the value carries no
   `CustomerTextMarkers` prefix — the string *is* the instruction, unlabelled, the #1058-leak-B / #803
   shape. The same customer's instruction body is correctly `[redacted]` on the recognized
   `dropoff_pre_arrival` four minutes earlier in the same delivery. *Hypothesis:* 8.97.8 routes
   leave-at-door through this wrapper page in addition to the pre-arrival card; #912 item 2 already lists
   the surface as unruled but never graded it as PII-bearing.
   - **Status:** Open (#1107 — Pledge; #912 item 2 is the recognition half).

2. **A return order walked off with $10.93 — a third of the dash — and the whole flow is UNKNOWN.**
   Session 542: an alcohol delivery was refused for an underage ID, DoorDash converted it to a return,
   and paid `Original order (Base) $21.85 + Return pay $10.93 = $32.78`
   (`UNKNOWN/2026-09-14_13-03-24-474…`). The state machine closed the job at 13:03:23 on the $21.85
   estimate; the return leg's **own** receipt arrived 10 minutes later
   (`delivery_summary_expanded/2026-09-14_13-13-49-895…`, `This offer $10.93 / Base pay $10.93`) with no
   open job — no `Saved:` bubble, no `#1033 receipt re-price`, no `DELIVERY_RECEIPT_REPRICE`. The session
   total is right (read off the summary); the row is short by exactly the return pay. Six UNKNOWN frames
   carry the `Return to store` card and the `Return by 13:04` chrome; thirteen of the pull's nineteen
   backstop scrubs come from them (all scrubbed — nothing leaked). *Hypothesis:* #998 recurring, now with
   a one-drop, one-figure instance that is trivially replayable.
   - **Status:** Open (#998; evidence comment posted).

3. **Nine of twelve `OFFER_TIMEOUT`s were declines — #1104's second sighting, and the dev flagged it
   independently the same day.** Per-timeout table in the desk report. The discriminator is unchanged and
   now measured nine times: the dasher's confirm tap lands **24–229 ms** (median 145) *before* the
   `offer_popup_confirm_decline` frame is admitted, so `doordash.click.decline_offer`'s
   `screenIs` gate reads `offer_popup` and the click falls to UNKNOWN. **Sharpening:** the confirm SHEET
   was recognized in time in every case — the `Denied confirm_decline` WARN 200–500 ms later proves the
   screen rule matched and reached the capability gate. So this is a race *between the click sub-pipeline
   and the window sub-pipeline*, not a recognition failure, which constrains the fix to the click's
   screen context. Counts: 58 `initial_decline` vs 47 `decline_offer` clicks; 94 `Denied confirm_decline`
   across **57 episodes** (37 double + 20 single — the pairs are the throttled retry, not one per
   decline). The three non-#1104 timeouts are genuine: two cards left to run 47.2 s and 48.0 s with no
   decline action at all, and one (seq 2275) where the decline was pressed at +44.7 s and the card
   expired 1.5 s later before any confirm frame.
   - **Status:** Open (#1104; second sighting, counts commented).

4. **A stacked job whose second drop never ARRIVED mixes mileage bases — Σ `realizedMiles` 24.32 mi
   against an 18.70 mi odometer span (session 499), and the sibling carries `realizedMinutes = −7.1`.**
   Job-…500's drop …501 has `milesToDropoff = 5.998` so it folds the leg basis (6.00 mi); drop …503 has
   no arrival and no `milesToDropoff` so it keeps the job/odometer basis (8.82 mi), and the shared leg is
   counted twice. Session/period/IRS totals are odometer-anchored and unaffected, but each row's frozen
   `netProfit` uses its own `realizedMiles`, so …503 is charged $3.20 of operating cost for a leg it
   shared. The negative-minutes row is the same drop's sibling; all **three** negative-minute rows in the
   lifetime database are the first drop of a two-drop job. *Hypothesis:* the per-drop minutes anchor
   belongs to the other drop of the stack, and the mileage basis is chosen per row instead of per job.
   - **Status:** Open (#1108).

5. **An order unassigned at the store through the pickup issue flow is recorded as a `PICKUP_CONFIRMED`,
   with no `TASK_UNASSIGNED` and an orphan accept.** Session 568: accepted $14.50, arrived 10:29:44, went
   `pickup_issue_menu → pickup_select_issue → pickup_resolution_options`
   (`pickup_resolution_options/2026-09-15_10-46-16-807…` reads `Unassign with no pay` / `Unassign order`),
   chatted with support, then `waiting_for_offer` at 10:47:09 — which the stepper read as an ordinary
   "left the task", armed a retire, and committed `PICKUP_CONFIRMED` seq 2529 with a **17.4-minute dwell
   sample** for a visit that ended in an unassign. Everything downstream behaved correctly given that
   input (no completion, no row, `storeKey` NULL, correctly no `JOB_ACCEPT_MISMATCH` because the drop was
   never ARRIVED — the documented #1101 limit), and `reportedEarnings` is correctly NULL. Only the pickup
   confirmation and its dwell are wrong. *Hypothesis:* #301 — the unassign flow is recognize-only, so the
   abandon is invisible to the lifecycle.
   - **Status:** Open (#301; evidence comment).

6. **#1102 is better but not fixed: 2 of 18 auto-taps still resolve against mid-slide bounds**
   (`top=4347` on 09-13 12:59:45, `top=4382` on 09-14 16:16:40, both far below a 2 400 px screen).
   16/18 clean = 89 %, against 6/11 = 55 % on 09-13 — the same defect, a luckier draw. No data lost:
   #1033 layer 1 caught both manual recoveries, one of them at **7.477 s**, the closest the 8 s window has
   ever come to expiring.
   - **Status:** Open (#1102).

7. **8.97.8 rotted the offer card's decline target anchors.** `bindShortfall{offer_popup#declineButton=8,
   offer_popup_confirm_decline#confirmDeclineButton=3}` plus three
   `No 'declineButton' target bound for doordash — decline_offer unavailable` (09-14 14:02:52, 09-15
   11:41:30, 09-15 11:46:53) — the #457 notification Decline button went unavailable three times and the
   dasher declined by hand each time. Separately, `parseShortfall{waiting_for_offer=180}` is the
   **loading** variant of the idle screen (`Looking for offers` + `loading_view`, no earnings pill at
   all), i.e. the benign baseline #1049 exists to declare — now the dominant census entry and swamping the
   signal.
   - **Status:** Open (logged; #1049 for the baseline — the decline-bind rot is not filed yet).

8. **A `navigation_generic` frame mid-route armed a premature retire and emitted a duplicate
   `DELIVERY_CONFIRMED` 13 minutes early** (session 564, seq 2511 at 18:23:45 vs the real arrival at
   18:24:47 and completion at 18:37:05). Row-inert, money correct — and a nice demonstration of #1078's
   round-4 arrival gate, which is exactly what stops a dash end honoring a retire like this one. It is
   also the source of the 25-vs-24 `DELIVERY_CONFIRMED`/`DELIVERY_COMPLETED` asymmetry.
   - **Status:** Open (logged, low — not filed).

9. **`earnings_deposit` persisted the DasherDirect/Crimson clause twelve more times** (09-13 ×4, 09-14
   ×7, 09-15 ×1), each on a RECOGNIZED envelope, while the identical marker on the UNKNOWN path was
   scrubbed (`Cr7` ×3). Same asymmetry, seventh pull. They are the **only** hits of the dasher-banking
   sweep over the whole tree.
   - **Status:** Open (#987, dev ruling queued).

10. **The `Tip Update` push is recognized now, but the $2.00 still lands nowhere.**
    `notification/additional_tip/2026-09-14_09-55-52-079…` names the amount, the merchant and the delivery
    time; `delivery_records` seq 2355 (McDonald's, 09:41:10) still reads `$8.25 OFFER_PAY, tip NULL`, and
    session 521's summary had already been read 13 minutes earlier. #1002's recognition half is done; the
    reconciliation half is the open one.
    - **Status:** Open (#1002; evidence comment).

### Verification (desk)

11. **#1078/#1095 CONFIRMED on its first field run, on a 0.385-second margin.** Session 492's last drop:
    `DELIVERY_ARRIVED` 11:36:14.381 → `dash_along_the_way` **11:36:22.296** (the `TASK_RETIRE` arm,
    deadline 11:36:32.296) → `dash_summary` **11:36:31.911** (the `SESSION_END` arm, **385 ms** before
    that deadline — the 09-08 loss had 1 614 ms) → `GRACE_COMMIT` 11:36:34.415 → `DELIVERY_COMPLETED`
    seq 2240 stamped `completedAt = 11:36:22.296`, i.e. the absorbed retire's own `since`, exactly as
    §Graces specifies. **$35.50 that the pre-#1100 build would have lost without a WARN is on the books.**
    The commit matured past the absorbed floor (34.417 ≥ 32.296) and `DASH_STOP`'s `endedAt` is unchanged
    at `since`. Across the whole pull: 24 delivered drops → 24 `DELIVERY_COMPLETED` → 24 rows, **zero
    `JOB_ACCEPT_MISMATCH`** (so #1095's false-positive half also passes), eleven summary ends at
    2.501–2.508 s and three early-offline ends at 10.002–10.003 s. The deliberate **< 10 s** end is still
    not done (tightest arrival→end 17.5 s — which happened to be this one).
12. **#1096 measured a second time and retired at 2/2.** Eleven receipt first beats, gap to the first
    recognized `delivery_summary_collapsed` **169–785 ms** (median ≈ 450 ms) — never near the ~2 s that
    would let the expand tap be scheduled off one. The wheel on those beats never reaches
    `sessionEarnings` because an UNKNOWN frame is never parsed; one of them
    (`UNKNOWN/2026-09-14_17-00-09-795…`) read `This dash so far $368.35` on a dash whose real total was
    $77.26. The `Send thanks to the customer for the tip!` chrome survives into 8.97.8.
13. **#992 CONFIRMED — the last unseen surface of the Pledge redact batch finally rendered.**
    `pickup_wait_survey/2026-09-14_09-32-36-330…` carries `customer_name = [redacted:3aa3]` with the order
    number, item list and store chrome intact. With #993/#994/#995/#920 already at 2/2, the batch can
    close.
14. **#1033 layer 1 set a new record and layer 2's stacked half is finally exercised.** One receipt
    expanded **7.477 s** after its collapsed frame (09-14 16:16:40 → 16:16:47.57) — 0.5 s inside the 8 s
    window, folded `DROP_SHARE $22.50`; the pre-#1033 2.5 s window would have lost it. Layer 2 fired 18×
    and no-op'd 18× (`row NNNN already holds N¢`), `receiptRepricedAt` NULL on all 24 rows, zero
    `no delivery row` WARN, `rows_total == receipt_total` on all 16 receipt-priced jobs. The stacked job
    emitted `2 drop(s), receipt 1315¢` and its two `dropRealizedPay` (657¢ + 658¢) sum to `totalPay` to
    the cent. The deliberate **> 8 s** expansion remains undone.
15. **Also working:** #1063's desk half a fourth time (82 offers → 82 `OFFER_RECEIVED` → 82 TTS → 82 chat
    cards → 82 screenshots → 82 resolutions, zero "Replaced by new offer" / "Superseded by direct
    offer"); #1103 no-regression (all three `early_offline` sessions have NULL `reportedEarnings`, and
    exactly one `SESSION_PAY_SETTLE` fired in three days — correctly inside its own session); #991
    no-regression (82/82 `speak()`, zero `-1`); #438 B4 (31 `OfferActionReceiver` taps, every hash
    matching its resolved row); #859 (82 offer screenshots 1:1, zero `{` filenames); #731 (one connect,
    zero disconnects, one process, three days); #159/#773/#1000 (six H-E-B running keys plus McDonald's
    `@5503`, zero `D6 join miss`, 23 of 24 accepted offers linked — the 24th is the unassigned one,
    fail-null); #315 H5 (23 dwells 0.0–42.8 min); #588 (`0.79/min n=104`, `items:units 0.78 n=51`, all
    `[doordash]`-tagged); #843's eighth corroboration (94 fail-closed `confirm_decline` denials beside a
    granted `expand_earnings`); #967's desk half a third time (`ratings.capturedAt` advanced to
    12:01:31 on an `[DoorDash:Offline]` browse); #986/#1031/#1039 (zero hex on any subpremise, 20 plain
    `Apt/Suite [redacted]`); #1057/#918 (four `POOR_ACCURACY` rejections, each a one-fix streak recovered
    in 1–7 s, 11 160 accepted / 837 ignored / 3 rejected, +379 km); #910 (dropoff-phase Google-Nav
    maneuvers masked on click envelopes too, while `pickup_navigation` stays raw by design);
    `IcuRegexGuard`-adjacent health — zero ERROR, zero pipeline restarts, zero mapping failures.
16. **Not exercised:** both #1054 items for a **fourth** pull (one process for the entire three days —
    startup 09-13 09:14:53 replayed 2 observations with no session and correctly logged no
    `Recovery re-armed`; zero `DASH_PAUSED`/`DASH_RESUMED`/`MODE_RESUME_COMMIT`/`SESSION_PAUSED_SAFETY`);
    #1058's workflow sheet a fourth time (`grep -c dropoff_workflow_sheet` = 0; four of the five check-1
    hits are the documented text-free skeletons, and the fifth is a real alcohol pre-arrival card the
    #1058 arm did not claim on 8.97.8 — no PII on that render); #1059 a fourth time (zero ids tree-wide,
    and the over-match check passes a fourth time: ~20 Red-Card shop orders, 211 `pickup_shopping` + 80
    `shopping_item` frames, zero `sensitive.red_card` drops — note the 16 `sensitive.id_verification`
    drops on 09-14 are the *pre-existing* licence-scanner anchors, which the log cannot distinguish from
    #1059's passport arm); #985 (the timeline order-detail sheet was never opened); #1034 (nothing
    negative); #936 (0 null distances, 0 `quality='UNKNOWN'`); #660 (zero null-session rows); #736/#752
    (zero `TASK_UNASSIGNED` — see Bug 5); #1000; every Uber item; every dev-eyes UI item.

### Open questions

17. **The `waiting_for_offer` loading variant is now 180 of the pull's parse shortfalls** — a screen that
    legitimately renders `Looking for offers` and nothing else. Until #1049 lands an `optional` field
    declaration, the #1036 rot signal for the idle earnings pill is unreadable: a genuine 8.97.8 wheel rot
    would be indistinguishable from this baseline.
18. **Every delivery completed into "Dash Along the Way" renders no receipt at all**, and that is where
    100 % of this pull's unexplained money sits: all six `OFFER_PAY` drops, and both the $3.71 (a
    46-item mid-shop pay adjustment DoorDash explicitly said it would only show "when the order is
    delivered") and the $3.75 deltas. #1035's idea — reconcile against DoorDash's own weekly Earnings
    screen — would close exactly this class, and so would folding the `additional_tip` push (#1002).
19. **Three new no-PII UNKNOWN families worth a cheap rule on 8.97.8:** the `Merchant delay protection`
    education sheet (14 frames), the `Couldn't scan ID / Scan again` alcohol rescan sheet (2), and the
    `Can't hand order to customer` contact-CX steps page (2). The `Underaged ID. Do not hand over any
    items.` HELP page (1) belongs with the return-order family in #998.
...
## 2026-09-10 → 2026-09-12 (desk analysis of the 09-13 pull — the receipt auto-tap is a coin flip; a stale receipt taught a new dash the old dash's total; a fifth of declines recorded as timeouts)

**Date:** 2026-09-10 → 2026-09-12 · **Platform(s) tested:** DoorDash (app **8.96.8**, every envelope) ·
**Branch under test:** `master` at `8028691a` (post-#1094; read off the logs —
`DashBuddy 0.230.0+8028691a starting (built 2026-09-08T15:59:51.877Z)`, 62 `PipelineStats` lines all
`app=0.230.0+8028691a`). **Pre-#1100**, so the #1078/#1095 dash-end fix is NOT on this build ·
**Field conditions:** seven DoorDash dashes over two days — 09-10: 452 (08:07–09:44 $35.20), 459
(09:44–10:04 $6.20), 463 (10:04–14:21 $124.43), 476 (15:26–17:08 $40.14), 483 (17:11–17:13
`early_offline`, no deliveries); 09-11: 484 (09:58–11:01 $22.70), 488 (11:02–12:17 $32.05). Eight
H-E-B shop-and-deliver orders plus Dunkin', Bill Miller BBQ and Sprouts; 31 offers in the log window
(11 accepted / 13 declined / 7 timed out); Uber Offline throughout; no pause, no force-stop, no
unassign, no orphan, no stacked job. 682 captures (82 UNKNOWN screens, 118 UNKNOWN clicks, 3 UNKNOWN
notifications, `unknownDropped` 318 / `forwarded` 2000 ≈ 13.7 %), **71 WARN / 0 ERROR**, no
`RecognitionHealth` / `LocaleBoundary` / `TtsHealth` notice, `shareable.log` PII-free. One capability
granted (the #1093 receipt-expand, carried over from 09-08); `confirm_decline` still denied
fail-closed 31×. Pull dir `~/dashbuddy/logs/2026/09/13`.

### Bugs

1. **The #1093 receipt auto-tap failed on 5 of 11 receipts — the bind's bounds are captured while the
   sheet is still sliding up.** 11 `Performing expand_earnings` fires, 6 clean
   (`Single verified candidate … clicking it`), 5 `Could not find any live node for: expand_earnings …
   (id=null, text=null, bounds=BoundingBox(left=36, top=T, …))` → `did not fire — target failed
   resolution/verification (fail closed)` → a throttled retry 570 ms later. The five `top` values are
   **4435 / 4395 / 4263 / 2224 / 1982** on a 1080×2400 screen whose settled `This offer` row sits at
   **1774–1890**. Two envelopes of the *same* receipt 641 ms apart pin it:
   `delivery_summary_collapsed/2026-09-10_10-03-34-464` has the row at `(36,4435)-(1044,4549)`,
   `…10-03-35-105` has it at `(36,1872)-(1044,1998)`. The intervening frame
   (10:03:34.782) was classified but never admitted — identity dedup — so the bind never refreshed,
   and `SETTLE_UI` fired at 10:03:34.969, 116 ms before the settled frame arrived.
   *Hypothesis:* `GraceConfig.EXPAND_SETTLE_MS = 500` (`GraceConfig.kt:79`, consumed at
   `EffectMap.kt:307/341`) is shorter than 8.96.8's ~590–640 ms prism-sheet slide, and
   `UiInteractionHandler.findNodeByBounds` (`:372-374`) needs IoU ≥ `RELAXED_BOUNDS_IOU` 0.5
   (`:67`) against a 126 px row — at most ~63 px of tolerable drift against a fielded 110 px
   (11:05), ~350 px (12:17) and ~2 500 px (the other three). The failure is `candidates.isEmpty()`
   (`:133-139`), never a label rejection: `NONE passed verification` and the nested-abort WARN
   fired zero times, so #1093's label-hint guards were not the blocker. Would need the two
   10:03:34/35 envelopes replayed through `SessionReplay.reduceMixed` with a synthetic `SETTLE_UI`
   at +500 ms and at +800 ms to show the tap resolving only in the second. **No data was lost** —
   #1033 layer 1's 8 s window caught the dev's manual tap on all five (expansions at 2.485–3.138 s).
   - **Status:** Open (#1102 — the #1093 regression).

2. **A dash that starts while the previous dash's receipt is still on screen inherits its running
   total — session 483 reported $40.14 on zero deliveries.** Dev's own words: *"a couple of times(?)
   i think, there was a post delivery screen shown just after i started a new dash, from the last
   dash"*, and *"i think the stepper actually ignored it. at least, it didn't log anything in the
   bubble that i remember."* **Both halves check out, and they split cleanly into two layers.**
   *(1) The task/job lifecycle was INERT* — `app_events` seq 2168→2175 for session 483 holds only
   `DASH_START`, three `OFFER_RECEIVED`, one `OFFER_TIMEOUT`, two `OFFER_DECLINED`, `DASH_STOP`. No
   `DELIVERY_*`, no `DELIVERY_RECEIPT_REPRICE`, no `[Earnings]: Saved:` chat, no
   `Session Ended. Total:` bubble, zero `delivery_records` rows. *(2) The session-end payload
   LEAKED* — `session_records` 483 carries `reportedEarnings = 40.14`, exactly session 476's total.
   Trail: `DASH_START` 17:11:36.403 → `SCREEN: delivery_summary_collapsed` **17:11:36.417 (14 ms
   later)**, the envelope `delivery_summary_collapsed/2026-09-10_17-11-36-440` reading
   `Total online time 1 hr 44 min · Offers accepted 2 out of 5 · This offer $24.39` (all session
   476's figures) → the #1093 auto-tap expands the stale sheet 17:11:37.191 → `Timer Expired:
   SESSION_PAY_SETTLE` **17:11:39.669** → `DASH_STOP` seq 2175
   `{"source":"early_offline","totalEarnings":40.14,"sessionDurationMillis":null,"offersAccepted":null}`.
   *Hypothesis:* NOT the `SessionEndedFields` path — that arm (`ModeEffects.kt:82-101`) stamps
   `source = SUMMARY_SCREEN` with all three companions populated and posts a total bubble, and no
   `dash_summary` frame was classified between 17:08:04 and 09-11 11:01:12. It is the settle gate:
   `PlatformRegionStepper.sessionPayRead()` (`:1181`) counts `PostTaskFields.sessionEarnings` as a
   running-total read, `settleSessionPay` (`:1116-1152`) parks it keyed on `(flow, platform)` with
   **no session identity** in `PendingSessionPay` (`PlatformRegion.kt:506-546`), `commitSessionPay`
   (`:1203-1211`) writes it into `region.session.runningEarnings` behind only a non-null check, and
   `ModeEffects.kt:136` then stamps `prevSession.runningEarnings.takeIf { it > 0.0 }`. The
   "never invented into a later dash" guard in `commitSessionPay`'s KDoc covers a park that
   *survives* a boundary; this park was *born* 267 ms inside the new dash off a screen describing
   the old one. Distinct from #1030 (fabricated `$0.00`) and #1041 (dead `0.0` feeders) — this is a
   fabricated **non-zero carried across a boundary**. The same frame shape recurred at the 484→488
   boundary (`DASH_START` 11:02:22.985 → receipt 11:02:23.236, `Total online time 1 hr 4 min`) and
   was a near-miss: no `SESSION_PAY_SETTLE` fired (R0 left the surface inside the 3 s window) and
   488 ended on the summary screen, whose authoritative write would have overwritten a stale value
   anyway. Would need the 17:07:48→17:14:01 envelope sequence replayed through
   `SessionReplay.reduceMixed` asserting `session-483.runningEarnings == 0.0`.
   - **Status:** Open (#1103).

3. **Five of the seven `OFFER_TIMEOUT`s are declines the app failed to recognize.** All seven
   payloads carry `description: null`, so none is the #830 replace or #881 direct-offer class. Five
   reached the confirm sheet but recorded a timeout: 10:07:19, 10:08:03, 16:03:30, 17:12:01 and
   09-11 10:09:03; only 15:27:46 and 09-11 10:17:28 are genuine expiries (cards left to run 46–48 s).
   The discriminator is a race: at 10:07 the dev's confirm tap lands `10:07:18.187 UNKNOWN click —
   id=null text=null` and the `offer_popup_confirm_decline` frame is admitted **171 ms later** at
   10:07:18.358; in the working case (09-11 10:17) the sheet is admitted at 10:17:47.361 and the tap
   at 10:17:47.810 captures as `target=decline_offer ruleId=doordash.click.decline_offer` →
   `Declining…` → `Offer Declined`. The two click envelopes'
   node payloads are **byte-identical** — an id-less `android.widget.Button` at `(36,2043)-(1044,2150)`
   whose only child is `textView_prism_button_title` = `"Decline offer"`; the sole difference is
   `"screenTarget"` (`offer_popup` vs `offer_popup_confirm_decline`). Corroborating counts: 18
   `initial_decline` clicks vs only 13 `decline_offer` clicks; 31 `Denied confirm_decline` = 13
   double-denial episodes + 5 single-denial ones, exactly.
   *Hypothesis:* `doordash.click.decline_offer` is gated `screenIs: "offer_popup_confirm_decline"`
   (`matchers/rules/doordash/offer.json5`), which reads the last **admitted** screen; a confirm tap
   inside ~200 ms of the app's own `initial_decline` tap beats that frame, the click falls to
   UNKNOWN, nothing latches a decline, and the per-offer `OFFER_EXPIRY` timer later resolves it as a
   timeout. Same "faster than the animation" family as item 1, one surface over. Why 09-08 saw 0
   timeouts over 14 declines is unexplained — plausibly tap rhythm, unproven. Downstream: the Offers
   tab funnel, the acceptance rate, and #864's bubble misread all inherit the wrong outcome.
   - **Status:** Open (#1104).

4. **The `going_to_store_map` pickup contact sheet still falls to UNKNOWN on 8.96.8 — backstop as the
   primary control (#1079 recurrence).** Two `WARN/Pipeline: Capture backstop: UNKNOWN screen carried
   customer PII (textMarker=- nodeId=address_line_1) — scrubbing node from envelope` at 09-10
   08:30:36.640 and 09-11 10:27:13.379; the envelopes
   (`UNKNOWN/2026-09-10_08-30-36-685`, `UNKNOWN/2026-09-11_10-27-13-409`) carry `mx_contact_view` +
   `address_instructions_view` + `order_items_view` + `customer_contact_view`, with both address
   nodes reading `[redacted]` on disk and `user_name` text-free on these renders. Nothing leaked; the
   posture is the one #1058 argued a rule must own.
   - **Status:** Open (#1079; evidence comment posted).

5. **`earnings_deposit` persisted the DasherDirect/Crimson clause six more times** (09-10 09:44:29,
   10:04:07, 14:21:02, 17:08:02; 09-11 11:01:13, 12:17:24), each on a RECOGNIZED envelope, while the
   identical marker on the UNKNOWN path was scrubbed (`Cr7`, 09-09 17:00). Same asymmetry, sixth pull.
   - **Status:** Open (#987, dev ruling queued).

### Verification (desk)

6. **Money reconciles to the cent on all six delivering sessions** — 452 $35.20, 459 $6.20, 463
   $124.43, 476 $40.14, 484 $22.70, 488 $32.05, each exactly `Σ (realizedPay + cashTip)`. 11 drops,
   11 `DELIVERY_COMPLETED`, 11 `delivery_records`, zero `JOB_ACCEPT_MISMATCH`, zero
   `OFFER_OUTCOME_CORRECTION`, zero unattributed. Σ `realizedMiles` ≤ odometer span on every session
   (15.90/15.91, 3.03/3.03, 46.19/46.21, 11.29/11.29, 18.25/18.26, 15.04/15.05) with both legs
   non-zero on every drop — #688B holds. The two `OFFER_PAY` rows are explained and benign: seq 2060
   had an `offer_popup` inflate over the receipt at 09:27:22.416, 73 ms before the retire grace
   committed (no receipt frame at all, no `Saved:` chat); seq 2193's receipt did not render until
   11:02:23, **85 s later and inside the next dash**, and correctly emitted no re-price. Both
   estimates happened to equal the real pay exactly (no post-quote tip), so nothing is off —
   coincidence, not guarantee.
7. **#1033 layer 1 keeps working; layer 2 fired 9 times and no-op'd 9 times.** Every receipt expanded
   within 1.244–3.138 s, all inside the 8 s collapsed grace; 9 of 11 drops folded `DROP_SHARE`. The
   nine `DELIVERY_RECEIPT_REPRICE` events (seq 2071/2086/2106/2116/2126/2136/2152/2166/2210) each
   resolved `row NNNN already holds N¢ — no-op`; `receiptRepricedAt` is NULL on all 11 rows; zero
   `DELIVERY_RECEIPT_REPRICE: no delivery row` WARN; `rows_total == receipt_total` on all nine
   receipt-priced jobs. **The deliberate > 8 s late expansion and the stacked job are still not done.**
8. **#1096 measured on every receipt and benign.** Eleven first-beat UNKNOWN envelopes; the gap to
   the first recognized `delivery_summary_collapsed` is **62–613 ms** (median ≈ 424 ms), never near
   the ~2 s that would let the expand tap be scheduled off it. The `$0.00` wheel appears on three of
   them and never reaches `sessionEarnings` — the frame is UNKNOWN and therefore never parsed. Same
   beats also carried `$47009.93`, `$145.75`, `$81.23`, `$50.00`: the mid-spin class is routine.
9. **Also working:** #1063's desk half a third time (31 offers → 31 `OFFER_RECEIVED` → 31 utterances
   → 31 screenshots → 31 chat cards → 31 resolutions, zero "Replaced by new offer"/"Superseded by
   direct offer"; the three offer-shaped UNKNOWN frames carry no `display_name`, the permitted #595
   half-renders); **#995 sighted for the first time** — `pickup_receipt_scan` rendered 09-10 09:32
   and is fully masked (`Focus on [redacted:c977]` + the scanned-name node `[redacted:c977]`, same
   hex = same customer, store/order chrome intact); #1031/#1039 a second time (zero raw subpremises,
   zero `Apt [redacted:<hex>]`, `Customer Notes: [redacted]` plain, `Entry code` label only);
   #992–#994 (zero `arriving_at_title`, zero `Return …`); **#1057/#918's first field rejection** —
   `Odometer fix rejected (POOR_ACCURACY): delta=9m dt=7131ms accuracy=226 m — entering a rejection
   streak` 17:04:02, `reception recovered after 1 rejected fixes over 24 s` 17:04:27, totals 7 166
   accepted / 633 ignored / 1 rejected / +233 742 m; #991 no-regression (31/31 `speak()`, zero `-1`,
   ladder untouched); #438 B4 (10 `OfferActionReceiver` taps, every hash exact-matching its resolved
   row); #859 (31 offer screenshots, 1:1, zero `{` filenames); #588 (`0.76/min n=84`, `items:units
   0.78 n=34`, all `[doordash]`-tagged); #731 (one connect, zero disconnects); #159/#773/#1000 (five
   H-E-B stores keyed `@5601`/`@5910`/`@7330`/`@12777` + Dunkin' `@2024`, Bill Miller `@8811`,
   Sprouts `@8101`, all with real running keys, zero `D6 join miss`, all 11 accepted offers linked);
   #843's seventh mechanism corroboration; #315 H5 (11 pickup dwells 1.9–65.9 min, minutes scale);
   the #1093 `bindShortfall` census earning its keep benignly (one WARN at 12:44:58.262 on a first
   beat that rendered `This offer` without its `Expand` chevron).
10. **Not exercised:** both #1054 items for a third pull (zero `DASH_PAUSED`/`DASH_RESUMED`/
    `MODE_RESUME_COMMIT`/`SESSION_PAUSED_SAFETY`; the only restart, 09-11 22:34, replayed 4
    observations with no session and correctly logged no `Recovery re-armed`; healthy baseline —
    six summary ends at 2.501–2.506 s, two early-offline ends at 10.003/10.005 s); #1078's
    deliberate < 10 s end (tightest gap 12.4 s, session 488; 484 is the near-miss at 14.1 s with the
    retire committing 4.1 s before the summary); #1058 (`grep -c dropoff_workflow_sheet` = 0, the
    two check-1 hits are the documented text-free skeletons); #1059 a third time (zero ids tree-wide,
    and the over-match check passes a third time — 8 shop orders, 99 `pickup_shopping` + 53
    `shopping_item` frames, zero `sensitive.red_card` drops); #1034 (nothing negative); #985; #936
    (0 null distances, 0 `quality='UNKNOWN'`); #967 (no ratings browse); every Uber item; every
    dev-eyes UI item.


**Retired from the checklist at 2/2 (desk) this entry:** #1031/#1039 (two consecutive pulls with zero raw subpremises, zero hex on a subpremise, `Customer Notes: [redacted]` plain, `Entry code` label-only; the uber residual stays unexercised and is noted under #1039). #1093 is MOVED here as BROKEN (Bug 1 → #1102) per the checklist rule; a fresh validation item returns with the fix.

### Open questions

11. **The DoorDash weekly Earnings screen is in the UNKNOWN pile with the dasher's own money
    history.** `UNKNOWN/2026-09-10_14-21-24` carries `Sep 7 - Sep 13`, `$279.97`, `Active time
    9h 30m`, `Dash time 10h 39m`, `Completed deliveries 11`, `Weekly goal $279.97/$380 (73%)` and a
    per-dash list with dates and amounts. No customer PII. Two defensible postures and the dev owns
    the call: *block it* (the standing note extends the sensitive class to the dasher's own earnings
    surfaces) or *recognize it* (it is the cleanest platform-authoritative reconciliation source the
    app has seen — literally the `reportedEarnings` column with dates, which is the ask in #1035 and
    the sibling of #923). What it must not stay is what it is: unruled, on disk, under neither
    posture. Note it would have caught item 2 instantly — it lists 09-10 as three dashes, not four.
12. **Three new no-PII UNKNOWN families worth a cheap rule** — the `Pickup steps / Scan receipt /
    Verify items` wrapper pages (2 frames, 09-10 09:32), the `Can't verify items` reason sheet
    (`Bag is sealed / Items not labeled / Staff intervened`, 2 frames), and the
    `Verification still in progress … / Got it / Enter code` pair (2 frames, 09-10 15:50). The last
    one's sibling "Enter code" screen deserves a Pledge look before a rule lands.
13. **Two duplicate evidence screenshots landed in the wrong session** as a side effect of item 2 —
    `2026-09-10 1711 Delivery - 24.39.png` and `…DeliveryBreakdown - 24.39.png`, second copies of
    session 476's receipt written 3.5 minutes later under session 483 (the rule's 60 s `throttleMs`
    had lapsed). Cosmetic, but it is the one user-visible trace the stale-frame class leaves.

---


## 2026-09-08 (desk analysis of the 09-09 pull — first field run of #1093, which works; a dash-end race silently ate a $21 drop)

**Date:** 2026-09-08 · **Platform(s) tested:** DoorDash (app **8.96.8** — moved from 8.95.6; every one of the
332 envelopes) · **Branch under test:** `master` at `8028691a` (post-#1094 merge; read off the logs —
`DashBuddy 0.230.0+8028691a starting`, built 2026-09-08T15:59:51Z, running from 11:00:25 with `restarts=0`;
the two `3ef5eff9` startups at the head of the first rotated log are 09-07 evening) · **Field conditions:**
three DoorDash dashes (15:44–17:43 $50.57 · 17:52–18:55 $23.00 · 19:06–19:13 `early_offline`), all three
H-E-B **shop-and-deliver** orders, the first dash entirely out-of-zone "Dash Along the Way" so DoorDash
rendered **no** delivery receipt for either of its drops; 17 offers / 3 accepted / 14 declined; Uber Offline
throughout; no pause, no force-stop, no unassign, no orphan. 332 captures (36 UNKNOWN screens, 25 UNKNOWN
clicks, 2 UNKNOWN notifications, 13.0 % UNKNOWN ratio), **42 WARN / 0 ERROR**, no `RecognitionHealth` /
`LocaleBoundary` / `TtsHealth` notice, `shareable.log` PII-free. The dev granted **one** capability at
11:05 — the #1093 receipt-expand — and nothing else. Pull dir `~/dashbuddy/logs/2026/09/09`; device purged
after the pull.

### Bugs

1. **Session −440 silently lost a delivered $21.00 drop; the dash-summary grace evicted the task retire —
   #1078 recurrence, mechanism now pinned.** Job −444 was accepted 16:52:01 ($21.00, offer `bf0fb9a1`),
   shopped (11 items), `PICKUP_CONFIRMED` 17:34:29, `DELIVERY_ARRIVED` 17:43:35, drop-off photo taken
   17:43:38 — and then `DASH_STOP` (seq 2007) and `DELIVERY_CONFIRMED` (seq 2008, `completedAt: null`) both
   landed at **17:43:54.709** with **no `DELIVERY_COMPLETED` and no `delivery_records` row**. The session
   reported $50.57 and recorded $29.57; the $21.00 gap is that offer's `payAmount` to the cent, and
   `offer_records` seq 2001 has `linkedJobId` NULL. The pickup half folded fine. Nothing WARNed.
   *Hypothesis:* the frame trail is `dropoff_photo` (`task:dropoff:arrived`) → **`dash_along_the_way`
   17:43:43.859** (`idle`), which arms `TASK_RETIRE` at +10 s = 17:43:53.859 (`TaskLifecycle.kt:446-467`) →
   `end_dash_confirm` (lifecycle-neutral) → **`dash_summary` 17:43:52.245** (`session:ended`), whose `else`
   arm in `PlatformRegionStepper.kt:653-684` builds a **fresh `SESSION_END` pending that REPLACES the live
   retire** 1.614 s before it would have committed (the `Mode.Offline` early-return at `:687-691` drops a
   `TASK_RETIRE` for the same reason) → `GRACE_COMMIT` 17:43:54.707 → `endSession` force-stamps
   `completedAt`, and `mintQualified` (`DeliveryCompletionEffects.kt:489-491`, judged against the PREV
   region) correctly refuses it because `retirePending` is now false (`:281`) — the T3 false-completion
   guard doing its job on evidence the machine had already thrown away. It is a **race, not a
   determinism**: the same shape two hours earlier (delivery 1, `dash_along_the_way` 16:51:06.625 →
   `GRACE_COMMIT` 16:51:16.620) completed normally because nothing armed a session end inside the retire
   window; the dev ended the dash 8.4 s after the drop instead of waiting. Would need the 17:43:35→17:43:56
   sequence replayed through `SessionReplay.reduceMixed` with a synthetic `GRACE_COMMIT` to confirm, and a
   second run with the summary frame moved past 17:43:53.859 to show the completion returning.
   - **Status:** Open (#1078 — second sighting, the cleaner repro; evidence comment posted).

2. **The #810 B1 accept-mismatch tripwire cannot see item 1.** `JobCloseEffects.kt:38-45` returns empty when
   `next.session == null` ("endSession is out of scope (v1)"), and job −444 closed through exactly that
   teardown — so a job with 1 accepted pay-bearing offer and 0 accounted drops produced no
   `JOB_ACCEPT_MISMATCH` and no WARN, and `session_records.deliveries` still reads 1. *Hypothesis:* the v1
   exclusion was aimed at the offline-mid-job / new-session mis-attribution case, which the session-identity
   test on the same line already handles; a dash-end teardown that strands an accepted offer is a different
   shape — the one the tripwire's own KDoc describes.
   - **Status:** Open (#1095).

3. **The "Delivery for" pickup contact sheet still falls to UNKNOWN on 8.96.8 — backstop as primary control
   (#1079 recurrence).** `UNKNOWN/2026-09-08_17-14-35-961__…__8d8cb4.json` (`customer_contact_view/user_name`
   under `going_to_store_map` + `Arrived at store`) was written only because the #910 `user_name` id backstop
   scrubbed the node first (`WARN/Pipeline: Capture backstop: UNKNOWN screen carried customer PII
   (nodeId=user_name)`). Nothing leaked — the name reads `[redacted]` and the three address nodes were
   text-free on this collapsed render — but this is the posture #1058 argued a rule must own.
   - **Status:** Open (#1079; evidence comment posted).

4. **New 8.96.8 receipt first beat lands in UNKNOWN.** `UNKNOWN/2026-09-08_18-53-00-789` reads `This dash so
   far · $ 0 . 0 0 · You got to 25% of your goal · Total online time 1 hr 0 min · Offers accepted 1 out of 4 ·
   Send thanks to the customer for the tip! · Send · Continue dashing` — the post-delivery sheet 656 ms before
   the recognized `delivery_summary_collapsed` frame, with no `This offer` row yet (so
   `allTextContainsAny: ['this offer', 'delivery complete']` cannot match) and new `Send thanks…` chrome absent
   from the corpus. Benign today; it is the same two-beat inflation shape as #1063's offer card.
   - **Status:** Open (#1096).

5. **`earnings_deposit` pushes stored "DoorDash Crimson account" twice more — still #987.** Both 09-08
   deposits persisted the clause on a RECOGNIZED envelope while, 12 h later, the identical marker on an
   UNKNOWN notification was scrubbed (`… hit sensitive marker id 'Cr7'`, 09-09 06:37:18) — the asymmetry in
   one pull.
   - **Status:** Open (#987, dev ruling queued; evidence comment posted).

### Verification (desk)

6. **#1093 CONFIRMED on its first field run, end to end.** The consent prompt appeared and the dev granted
   exactly one key — `f5e440da78d318ee0c579e216a9e6be1…`, which recomputes from the generated ruleset as
   `doordash.screen.delivery_summary_collapsed / expand_earnings / expandButton`. On the dash's one receipt:
   `SETTLE_UI` 18:53:02.074 → `INFO/Effects: Performing expand_earnings` → `attempting verified click` →
   `DEBUG/Effects: Single verified candidate … clicking it` (18:53:02.439) → `delivery_summary_expanded`
   18:53:02.945. **Collapsed→expanded 1.500 s** — the auto-tap shape, not the 2.5–3 s of the 09-05/06/07
   manual taps. No `Could not find any live node` / `NONE passed label verification` / aborted tie, and
   `bindShortfall{}` never named `delivery_summary_collapsed.expandButton` (only the expected
   `offer_popup#declineButton`). The drop folded `DROP_SHARE` with tip $10.00 / base $13.00 / $23.00.
   Only ONE receipt occurred — the other two drops were out-of-zone with no receipt at all.

7. **#1029's mid-spin guard fired in the wild on the receipt's own wheel.** The first collapsed frame
   (18:53:01.445) rendered `$ 2 9 3 . 0 9 0` — out of `CurrencyShape`, rejected — and the re-render 121 ms
   later read `$23.00` and won (`sessionEarningsAtCompletion: 23.0`), which is the `PostTaskFields.dedupeHash`-
   folds-`sessionEarnings` admission path working. Two more mid-spin reads on `idle_map`
   (`$ 8 9 9 1 . 3 6 0`, `$ 0 1 0 1 9 4 . 6 9 0`) confirm the class is routine on 8.96.8. The settle TIMER
   never armed (zero `SESSION_PAY_SETTLE`): session 440 showed no `waiting_for_offer` frame at all and
   session 447's three reads agreed with the total already in state.

8. **Also working:** #1033 layer 1 (2nd — **RETIRED at 2/2**; the "Saved: $23.00" bubble still fires on the
   receipt frame at 18:53:01.466, 21 ms after it appears) with layer 2 firing and correctly no-op'ing
   (`row 2024 already holds 2300¢`) — the deliberate >8 s late expansion was NOT done, layer 2 stays 1/2;
   #1063 desk half (17 offers → 17 `OFFER_RECEIVED` → 17 utterances → 17 screenshots → 17 resolutions, zero
   timeouts, zero "Replaced by new offer"; the two offer-shaped UNKNOWN frames are store-leg-less #595
   half-renders — **2/2 desk, dev-eyes half still open**); #1030 (session 451 reported nothing rather than a
   fake $0.00); #1057/#918 (1 754 accepted / 146 ignored / **0 rejected**, +56 682 m against 36.94 mi of
   span); #967 (`ratings.capturedAt` advanced to 18:57:32 on an **Offline** browse — **2/2 desk**, dev-eyes
   half still open); #438 B4 (10 `OfferActionReceiver` taps, all 10 hashes exact-matching their
   `OFFER_DECLINED` row); #859 (17 offer screenshots, one per presentation, every filename carrying a real
   figure); #588 (`0.76/min n=73→75`, `items:units 0.79 n=28→30`, all `[doordash]`-tagged); #731 (one
   connect per process, zero disconnects); #159/#773/#1000 (three H-E-B stores keyed `@5601`/`@5910`/`@7330`
   with real running keys, zero `D6 join miss`); #688B (Σ `realizedMiles` ≤ span on both sessions, both
   legs non-zero); #843 (`none granted — awaiting consent` on both starts, then one explicit grant — no
   auto-grant); the #1031/#1039/#992–#995 redact greps (zero raw names, streets, subpremises, codes or notes
   anywhere; every subpremise, ZIP, note and the one `Entry code` block on the six `dropoff_pre_arrival`
   envelopes reads PLAIN `[redacted]`; the only raw addresses are the two H-E-B **merchant** lines on
   `pickup_navigation`, raw by design per #886); money reconciles to the cent on sessions 447 and 451.

9. **Not exercised:** both #1054 items (no pause, no force-stop, no recovery with a live grace — the only two
   restores were `no tail` with no session; the two summary-ended `DASH_STOP`s show `occurredAt − endedAt` =
   2.508 s / 2.575 s, the ordinary 2.5 s window, healthy); #1058 (`grep -c dropoff_workflow_sheet` = 0;
   check 1's three hits are the documented text-free-skeleton false positives, second pull running); #1059
   (no Persona / Red Card wallet / passport — though the OVER-match check passes again: two `Red Card` shop
   offers and three shop pickups produced zero `sensitive.red_card` drops); #1034 (nothing went negative);
   #991 (17/17 utterances, ladder untouched); #985; #736/#752/#749/#660p2/#501/#526; every Uber item; every
   dev-eyes UI item.

### Open questions

10. **Two `CONFIRM_DECLINE` intents per decline.** 28 `Denied confirm_decline … (fail closed)` over 14
    declines — exactly 2 each, because `EffectMap.diffConfirmDeclineAction` schedules one `SETTLE_UI` per
    admitted confirm frame with no dedupe and the sheet is admitted twice (before and after the dev's own
    tap). The denial itself is right (the capability was never granted, and `SideEffectEngine.kt:362` checks
    it before the quick-declines setting at `:375`). With the capability granted the second tap would land
    on a dismissed dialog and abort at verification — noise rather than harm (28 of the dash's 42 WARNs).
    - **Status:** Open (#1097).

11. **DoorDash 8.96.8.** The #1029 re-anchors, the #1093 expand bind and every redact held across the bump.
    The new shapes are items 3 and 4 above; the 09-09 corpus intake captures the slice.
