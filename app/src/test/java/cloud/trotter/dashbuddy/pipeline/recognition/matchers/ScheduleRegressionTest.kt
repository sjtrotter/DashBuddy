...
class ScheduleRegressionTest(filename: String, node: UiNode) :
    BaseParameterizedTest(filename, node) {

    override val stats = sharedStats

    companion object {
        private const val FOLDER = "SCHEDULE_VIEW"
    ...
    fun `validate snapshot`() {
        val matcher = ScheduleMatcher()

        runTest(matcher) { result ->
            val info = result as ScreenInfo.Simple
            assertEquals(Screen.SCHEDULE_VIEW, info.screen)
        }
    }
}
