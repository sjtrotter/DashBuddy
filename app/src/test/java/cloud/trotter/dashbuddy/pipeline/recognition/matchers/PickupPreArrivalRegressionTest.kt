...
class PickupPreArrivalRegressionTest(filename: String, node: UiNode) :
    BaseParameterizedTest(filename, node) {

    override val stats = sharedStats

    companion object {
        private const val FOLDER = "PICKUP_DETAILS_PRE_ARRIVAL"
    ...
    fun `validate snapshot`() {
        ...
        runTest(matcher, parser) { screen, result ->
            ...
            assertEquals(Screen.PICKUP_DETAILS_PRE_ARRIVAL, screen)
            assertNotNull("Store name must be present", info.storeName)
            assertNotNull("Address must be present", info.storeAddress)
