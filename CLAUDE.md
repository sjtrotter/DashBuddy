...
`MAX_DELTA_WITHOUT_TIME_METERS` 2 000 when either fix carries no timestamp — `Coordinates` gained
nullable `accuracyMeters`/`timestampMs`, which `FusedLocationDataSource` had been dropping on the
floor) and returns Accept / Reference / Ignore / Reject: **neither an Ignore nor a Reject moves the
reference**, so a teleport anchors nothing AND slow creep inside the jitter floor still accumulates as
one later Accept, and a poor-accuracy fix cannot even seed the reference (fail-null). The repository
owns only the side effects — a WARN per reject carrying **numbers and the reason enum only** (a
latitude/longitude is the dasher's location PII and is never logged, at any level — principle 7) plus
ONE bounded DEBUG `Odometer fixes:` summary per 100 judged fixes. **Standing residual:** the gate is
