...
`minOf`; a PostTask frame that parses NO receipt at all keeps the pre-#1033 timing. Layer 1 is the ONLY path that lands an expansion in the STACKED shape (§5's re-price refuses it). A `SESSION_END` arming over a live `TASK_RETIRE` ABSORBS it (`absorbedRetireSince`) and `endSession` honors it as a completion (#1078) — but ONLY a provenanced retire (`absorbableRetireSince`: `armedFromFlow` non-null and neither `OfferPresented` nor `PostTask`), and a `task:unassigned` frame on the commit clears it; an arrival-only "rule 2" was REJECTED (fail-null). The #810 tripwire covers every close edge, attributed to the job's own session, single-accept floor lifted only on the lost-drop shape — session ended ∧ no qualified completion ∧ an ARRIVED drop (#1095).
