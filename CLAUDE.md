...
`minOf`; a PostTask frame that parses NO receipt at all keeps the pre-#1033 timing. Layer 1 is the ONLY path that lands an expansion in the STACKED shape (§5's re-price refuses it). A `SESSION_END` arming over a live `TASK_RETIRE` ABSORBS it (`absorbedRetireSince`) and `endSession` honors it as a completion (#1078) — but ONLY through `absorbableRetireSince`: a provenanced retire (`armedFromFlow` non-null, neither `OfferPresented` nor `PostTask`) over an ARRIVED, un-unassigned DROPOFF (#615), and a `task:unassigned` frame — on the commit OR in-window — disowns it (clears the value + marks the task); an arrival-ONLY "rule 2" was REJECTED (fail-null). A completion the PostTask exit already minted is recorded per task (`JobReceiptAnchors.exitMintedTaskIds`, written from the emitter's own `exitMintCandidate`) so the teardown never re-emits it. Every close-step emitter names the session through one rule (`closingSession`: `next` while it survives, `prev` when it ends) while NEW-task edges keep the live read. The #810 tripwire covers every close edge, single-accept floor lifted only on the lost-drop shape — session ended ∧ no qualified completion ∧ an ARRIVED drop (#1095).
