...
(#691). **That split is per-OFFER and consolidation-aware since #996/#997 — a pure mint-site change, no
fold touch, no `PROJECTOR_VERSION` bump, historical rows refold byte-identically.** `OfferPayFallback.shareFor`
now runs three arms in order: (1) the #996 **eligible-owed filter** — when the mint site proves the job
complete (`isJobPhysicallyComplete`, the #749 per-customer coverage arm IS the consolidation proof), a
never-activated customer-TBD placeholder leaves the denominator so its share lands on the drops that
exist (08-07: a \$13.10 two-order one-customer job folded 13.10/2 on its SOLE drop and the reconciliation
read reported−attributed = exactly the missing \$6.55); completion unproven (abandon, endSession bail,
mid-flight) keeps today's conservative dilution, and an **UNASSIGNED** order is never filtered (the
`owedDropoffs` doctrine — quoted but maybe unpaid); (2) the #997 **per-offer partition** — new additive
`Task.mintedByOfferHash` (stamped by `JobAcceptFlow.preCreatedDropoffs` at fresh-mint AND add-on
absorption; **slot identity**, so `swapTaskAccumulation` deliberately does not move it) partitions the
eligible drops per accept and splits each offer's OWN `payAmount` across its own partition, cents-exact
per partition (08-06: three accepts of \$10.45/\$16.55/\$20.20 absorbed into one job each folded the
pooled \$15.73 — a per-drop error up to \$5.28 that session-level Σ hides completely); a pay-less offer's
partition gets nothing and no longer dilutes its siblings; (3) the **pooled degrade** — ANY unresolvable
lineage (recovered/pre-#997 snapshot, a hash naming no accepted offer) falls back WHOLESALE to the
pre-#997 job-wide equal split, bit-exact. Degrading to null was rejected: `offerPayShare` is a declared
ESTIMATE basis and "known Σ, unknown split" IS its semantics, so pooling is the honest degraded answer.
**Σ invariant:** the job's Σ never exceeds the accepted quotes — for #997 it is unchanged (per-offer only
redistributes within the job, so period gross/`unattributedPay` cannot move); for #996 it RISES to the
full quote, moving dollars out of the unattributed bucket. `DeliveryPayload.mintedByOfferHash` rides the
log as the per-drop↔offer join it lacked (`jobOfferHashes` carries the whole add-on CHAIN on every row) —
...
`DELIVERY_ADJUSTMENT`), a mid-stack pay-less PostTask exit still rides unattributed forever (pre-existing
#691 FIX-1), and a cross-offer same-customer consolidation leaves the collapsed offer's partition
unattributed (WARN'd) rather than guessing a re-attribution. Miles from
