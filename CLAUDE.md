...
`minOf`; a PostTask frame that parses NO receipt at all keeps the pre-#1033 timing. Layer 1 is the ONLY path that lands an expansion in the STACKED shape (§5's re-price refuses it). A `SESSION_END` arming over a live `TASK_RETIRE` ABSORBS it — both its `since` AND its `deadline`, which floors the end's own so a commit is never pulled ahead of the retire's window (and re-bases with it on recovery) — and `endSession` honors it as a completion once that deadline MATURES (#1078), stamping `completedAt` at the absorbed instant and MARKING the task (`Task.honoredRetireAt`) — that mark, never the stamp's value, is how `mintQualified` recognizes an honored completion (equal timestamps made the force-stamp indistinguishable), so every stepper refusal turns the mint off by construction. Only through `absorbableRetire`: a provenanced retire (`armedFromFlow` non-null, neither `OfferPresented` nor `PostTask`) over an ARRIVED, un-unassigned DROPOFF (#615); a `task:unassigned` frame — on the commit OR in-window — disowns it (clears the value + marks the task); an arrival-ONLY "rule 2" was REJECTED (fail-null), as are the two residuals #1101 tracks (a missed arrival frame, a null-provenance restored retire). `JobReceiptAnchors.exitMintedTaskIds` records a PostTask exit's mint per task (written from the emitter's own `exitMintCandidate`) and is #1095 tripwire EVIDENCE only, never a mint gate — a gate would fail toward a lost row under a crash, while a duplicate raw emission is one durable row. Every close-step emitter names the session through one rule (`closingSession`: `next` while it survives, `prev` when it ends) while NEW-task edges keep the live read. The #810 tripwire covers every close edge, single-accept floor lifted only on the lost-drop shape — session ended ∧ no qualified completion ∧ an ARRIVED drop (#1095).
